...
ObjectPropItem::ObjectPropItem(QObject *object, ObjectsList *objects, const QString &name, const QString &displayName,
                               ObjectPropItem *parent, bool isClass)
    : m_object(object), m_name(name), m_displayName(displayName), m_haveValue(false), m_parent(parent),
      m_colorIndex(-1), m_readonly(true), m_model(0), m_isClass(isClass), m_changingValue(false) {
...
ObjectPropItem::ObjectPropItem(QObject *object, ObjectsList *objects, const QString &name, const QString &displayName,
    ...
    : m_object(object), m_name(name), m_displayName(displayName), m_value(value), m_haveValue(true), m_parent(parent),
      m_colorIndex(-1), m_readonly(readonly), m_model(0), m_isClass(false), m_changingValue(false) {
  if (parent) setModel(parent->model());
  m_index = QModelIndex();
  if (objects) foreach (QObject *item, *objects)
      m_objects.append(item);
...
void ObjectPropItem::appendItem(ObjectPropItem *item) {
  ...
  if (m_parent and (!item->isClass())) m_parent->m_globalPropList.append(item);
}

void ObjectPropItem::sortItem() { qSort(m_childItems.begin(), m_childItems.end(), lesThen); }
...
void ObjectPropItem::slotPropertyChanged(const QString &name, QVariant, QVariant newValue) {
  if (name.compare(propertyName(), Qt::CaseInsensitive) == 0 and !isValueChanging()) {
...
void ObjectPropItem::slotPropertyObjectName(const QString &oldValue, const QString &newValue) {
  Q_UNUSED(oldValue)
  if (propertyName().compare("objectName", Qt::CaseInsensitive) == 0 and !isValueChanging()) {
    setPropertyValue(newValue);
  }
}
#endif
...
bool ObjectPropItem::paint(QPainter *, const QStyleOptionViewItem &, const QModelIndex &) { return false; }
