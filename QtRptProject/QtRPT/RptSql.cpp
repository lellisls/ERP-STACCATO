...
RptSql::RptSql(QString dbType, QString dbName, QString dbHost, QString dbUser, QString dbPassword, int dbPort,
               QString dbConnectionName, QObject *parent)
  : QObject(parent) {
  /*#ifdef QT_DEBUG
  ...
  db = QSqlDatabase::addDatabase(dbType, dbConnectionName.isEmpty() ? QLatin1String(QSqlDatabase::defaultConnection)
                                                                    : dbConnectionName);
...
bool RptSql::openQuery(QString sql, QString dbCoding, QString charsetCoding) {
  ...
  if (not dbCoding.isEmpty()) {
    if (db.driverName().contains("MYSQL")) query->exec("set names '" + dbCoding + "'");
  }
  if (not charsetCoding.isEmpty()) {
    QTextCodec *codec;
