...
public:
  ...
  explicit ConnectionDesc(QObject *parent = 0);
...
public:
  ...
  void setLastError(QString value) {
    m_lastError = value;
    if (m_query) {
      delete m_query;
      m_query = 0;
...
public:
  ...
  bool isInvalid() const { return QueryHolder::isInvalid(); /*or m_invalid;*/ }
protected:
  void extractParams();
...
public:
  FieldMapDesc() {}
  explicit FieldMapDesc(FieldsCorrelation correlation) : m_fieldsCorrelation(correlation) {}
...
class MasterDetailProxyModel : public QSortFilterProxyModel {
  Q_OBJECT
public:
  explicit MasterDetailProxyModel(DataSourceManager *dataManager) : m_maps(0), m_dataManager(dataManager) {}
...
public:
  ...
  IDataSource *dataSource(IDataSource::DatasourceMode mode = IDataSource::RENDER_MODE);
  bool isOwned() const { return true; }
  bool isInvalid() const { return !m_lastError.isEmpty() or m_invalid; }
  bool isEditable() const { return true; }
  bool isRemovable() const { return true; }
  QString lastError() const { return m_lastError; }
  void invalidate(IDataSource::DatasourceMode mode);
