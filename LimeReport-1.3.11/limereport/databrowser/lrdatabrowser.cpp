...
DataBrowser::DataBrowser(QWidget *parent)
    : QWidget(parent), ui(new Ui::DataBrowser), m_report(0), m_closingWindows(false), m_settings(0),
...
DataBrowser::~DataBrowser() {
  delete ui;
  if (m_settings and m_ownedSettings) delete m_settings;
}
...
void DataBrowser::updateDataTree() {
  ...
  for (QString dataSourceName : m_report->datasourcesNames()) {
    ...
    QTreeWidgetItem *parentItem =
        findByNameAndType(m_report->dataManager()->connectionName(dataSourceName), DataBrowserTree::Connection);
    if (parentItem) {
...
QTreeWidgetItem *DataBrowser::findByNameAndType(QString name, int itemType) {
  ...
  if (!items.isEmpty()) {
    for (int i = 0; i < items.count(); i++) {
      if ((items.at(i)->type() == itemType) /* and (items.at(0)->text(0)==name)*/) {
        return items.at(i);
      }
    }
  }
...
QString DataBrowser::getVariable() {
  if (ui->variablesTree->currentItem() and ui->variablesTree->currentItem()->type() == DataBrowserTree::Variable) {
...
void DataBrowser::slotChangeConnection() {
  if (!getConnectionName().isEmpty()) {
    ConnectionDialog *connectionEdit =
        new ConnectionDialog(this, m_report->dataManager()->connectionByName(getConnectionName()), this);
    connectionEdit->setAttribute(Qt::WA_DeleteOnClose, true);
#ifdef Q_OS_MAC
    connectionEdit->setWindowModality(Qt::WindowModal);
#else
...
void DataBrowser::addSubQuery(SQLEditResult result) {
  try {
    m_report->dataManager()->addSubQuery(result.datasourceName, result.sql, result.connectionName,
...
void DataBrowser::addProxy(SQLEditResult result) {
  try {
    m_report->dataManager()->addProxy(result.datasourceName, result.masterDatasource, result.childDataSource,
                                      result.fieldMap);
  } catch (ReportError &exception) {
    qDebug() << exception.what();
  }
...
void DataBrowser::on_dataTree_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous) {
  Q_UNUSED(previous)
  if (current and (current->type() == DataBrowserTree::Connection)) {
...
void LimeReport::DataBrowser::on_addVariable_clicked() {
  ...
  QObject::connect(&dialog, &LRVariableDialog::signalVariableAccepted, this,
                   &LimeReport::DataBrowser::slotVariableEditorAccept);
  dialog.exec();
}
...
void DataBrowser::on_variablesTree_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous) {
  ...
  if (current) {
    if (m_report->dataManager()->containsVariable(current->text(1)) and
...
void DataBrowser::on_errorMessage_clicked() {
  if (!getDatasourceName().isEmpty() and m_report->dataManager()->dataSourceHolder(getDatasourceName())) {
