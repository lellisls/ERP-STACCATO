...
void Chart::setParams(bool drawGrid, bool drawHistory, bool drawCaption, bool drawGraphCaption, bool showPercent,
...
GraphParamList Chart::getGraphParamList() { return listOfGraph; }
...
void Chart::paintChart(QPainter *painter) {
    ...
    for (int i = 0; i < listOfGraph.size(); i++) {
      ...
      if (m_drawHistory) {
        QRectF textRect(m_left + this->width() - m_rightSpaceWidth + 20 * m_koefRes_w,
                        m_top + (100 + (i + 1) * 20) * m_koefRes_h, m_rightSpaceWidth,
                        20 * m_koefRes_h); // x y w h
        QRectF pointRect(m_left + this->width() - m_rightSpaceWidth + 25 * m_koefRes_w,
                         m_top + (103 + (i + 1) * 20) * m_koefRes_h, 10 * m_koefRes_w,
  ...
  // draw line X
  painter->drawLine(m_left + leftSpaceWidth, m_top + this->height() - 1 - m_bottomSpaceHeight,
  ...
  painter->drawText(m_left + this->width() - m_rightSpaceWidth + 30 * m_koefRes_w,
                    m_top + this->height() - 1 - m_bottomSpaceHeight + 5 * m_koefRes_h, "X");

  // draw line Y
  painter->drawLine(m_left + leftSpaceWidth, m_top + this->height() - 1 - m_bottomSpaceHeight, m_left + leftSpaceWidth,
                    m_top + captionHeight - 10 * m_koefRes_h);
  painter->drawText(m_left + leftSpaceWidth - 2 * m_koefRes_w, m_top + captionHeight - 15 * m_koefRes_h, "Y");
