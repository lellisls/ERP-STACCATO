...
ItemsBordersEditorWidget::ItemsBordersEditorWidget(ReportDesignWidget *reportEditor, const QString &title,
                                                   QWidget *parent)
...
void ItemsBordersEditorWidget::properyChangedEvent(const QString &property, const QVariant &oldValue,
...
void ItemsBordersEditorWidget::allBordesClicked() {
  int borders =
      BaseDesignIntf::LeftLine | BaseDesignIntf::RightLine | BaseDesignIntf::TopLine | BaseDesignIntf::BottomLine;
...
void ItemsBordersEditorWidget::buttonClicked(bool) {
  if (!m_changing and reportEditor()) reportEditor()->setBorders(createBorders());
}
...
void ItemsBordersEditorWidget::initEditor() {
  ...
  connect(m_topLine, &QAction::toggled, this, &ItemsBordersEditorWidget::buttonClicked);
  addAction(m_topLine);
  ...
  connect(m_bottomLine, &QAction::toggled, this, &ItemsBordersEditorWidget::buttonClicked);
  ...
  connect(m_leftLine, &QAction::toggled, this, &ItemsBordersEditorWidget::buttonClicked);
  ...
  connect(m_rightLine, &QAction::toggled, this, &ItemsBordersEditorWidget::buttonClicked);
  ...
  connect(m_noLines, &QAction::triggered, this, &ItemsBordersEditorWidget::noBordesClicked);
  addAction(m_noLines);

  m_allLines = new QAction(tr("All borders"), this);
  m_allLines->setIcon(QIcon(":/report/images/allLines"));
  connect(m_allLines, &QAction::triggered, this, &ItemsBordersEditorWidget::allBordesClicked);
