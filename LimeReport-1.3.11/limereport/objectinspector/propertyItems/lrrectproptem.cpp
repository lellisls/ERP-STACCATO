...
namespace {
LimeReport::ObjectPropItem *createReqtItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects,
...
LimeReport::ObjectPropItem *createReqtMMItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects,
                                             const QString &name, const QString &displayName, const QVariant &data,
                                             LimeReport::ObjectPropItem *parent, bool readonly) {
  return new LimeReport::RectMMPropItem(object, objects, name, displayName, data, parent, readonly);
}
bool registredRectProp = LimeReport::ObjectPropFactory::instance().registerCreator(
    LimeReport::APropIdent("QRect", ""), QObject::tr("QRect"), createReqtItem);
bool registredRectFProp = LimeReport::ObjectPropFactory::instance().registerCreator(
...
LimeReport::RectMMPropItem::RectMMPropItem(QObject *object, ObjectsList *objects, const QString &name,
                                           const QString &displayName, const QVariant &value, ObjectPropItem *parent,
                                           bool /*readonly*/)
  ...
  if (band) {
    ...
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, objects, "width", tr("width"), rect.width() / 10, this, true));
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, objects, "height", tr("height"), rect.height() / 10, this, false));
  } else if (page) {
    ...
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, 0, "width", tr("width"), rect.width() / 10, this, false));
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, 0, "height", tr("height"), rect.height() / 10, this, false));
  } else {
    ...
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, objects, "width", tr("width"), rect.width() / 10, this, false));
    this->appendItem(
        new LimeReport::RectMMValuePropItem(object, objects, "height", tr("height"), rect.height() / 10, this, false));
  }
  LimeReport::BaseDesignIntf *item = dynamic_cast<LimeReport::BaseDesignIntf *>(object);
  if (item) {
    connect(item, &BaseDesignIntf::geometryChanged, this, &RectMMPropItem::itemGeometryChanged);
    connect(item, &BaseDesignIntf::posChanged, this, &RectMMPropItem::itemPosChanged);
  }
}
...
LimeReport::RectMMValuePropItem::RectMMValuePropItem(QObject *object, ObjectsList *objects, const QString &name,
...
void LimeReport::RectMMValuePropItem::setModelData(QWidget *propertyEditor, QAbstractItemModel *model,
  ...
  object()->setProperty(parent()->propertyName().toLatin1(),
                        modifyRect(rect, propertyName(), propertyValue().toReal() * 10));
}
