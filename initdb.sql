...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_orcamento` (`Código` INT, `Vendedor` INT, `Cliente` INT, `Tel` INT, `Cel` INT, `Tel Com` INT, `Data` INT, `Total` INT, `Status` INT, `Dias restantes` INT, `Data Próx Followup` INT, `Observação` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_resumo_relatorio` (`Loja` INT, `Vendedor` INT, `Validos Anteriores` INT, `Gerados Mes` INT, `Revalidados Mes` INT, `Fechados Mes` INT, `Perdidos Mes` INT, `Validos Mes` INT, `mes` INT);
...
VIEW `mydb`.`view_orcamento` AS
    ...
    FROM
        ...
        LEFT JOIN `mydb`.`orcamento_has_followup` `fl1` ON ((`o`.`idOrcamento` = `fl1`.`idOrcamento`)))
        LEFT JOIN `mydb`.`orcamento_has_followup` `fl2` ON (((`fl1`.`idOrcamento` = `fl2`.`idOrcamento`)
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`profissional` (`idProfissional`, `idUsuarioRel`, `pfpj`, `nome_razao`, `nomeFantasia`, `cpf`, `cnpj`, `inscEstadual`, `contatoNome`, `contatoCPF`, `contatoApelido`, `contatoRG`, `tel`, `telCel`, `telCom`, `idNextel`, `nextel`, `email`, `tipoProf`, `banco`, `agencia`, `cc`, `nomeBanco`, `cpfBanco`, `desativado`, `created`, `lastUpdated`) VALUES (1, NULL, NULL, 'Não há', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, DEFAULT, DEFAULT, DEFAULT);
