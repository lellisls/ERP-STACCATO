...
bool Segment::intersect(Segment value) {
  return ((value.m_end >= m_begin) and (value.m_end <= m_end)) or
         ((value.m_begin >= m_begin) and (value.m_end >= m_end)) or
         ((value.m_begin >= m_begin) and (value.m_end <= m_end)) or
         ((value.m_begin < m_begin) and (value.m_end > m_end));
}

qreal Segment::intersectValue(Segment value) {
  if ((value.m_end >= m_begin) and (value.m_end <= m_end)) {
    return value.m_end - m_begin;
  }
  if ((value.m_begin >= m_begin) and (value.m_end >= m_end)) {
...
BandDesignIntf::BandDesignIntf(BandsType bandType, const QString &xmlTypeName, QObject *owner, QGraphicsItem *parent)
    : BaseDesignIntf(xmlTypeName, owner, parent), m_bandType(bandType), m_bandIndex(static_cast<int>(bandType)),
      m_dataSourceName(""), m_autoHeight(true), m_keepBottomSpace(false), m_parentBand(0), m_parentBandName(""),
      m_bandMarker(0), m_tryToKeepTogether(false), m_splitable(false), m_keepFooterTogether(false),
      m_maxScalePercent(0), m_sliceLastRow(false), m_printIfEmpty(false), m_columnsCount(1), m_columnIndex(0),
      m_columnsFillDirection(Horizontal), m_reprintOnEachPage(false), m_startNewPage(false), m_startFromNewPage(false) {
...
void BandDesignIntf::paint(QPainter *ppainter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
  ...
  if (itemMode() & DesignMode) {
    ...
    // bandNameRects.push_back(QRectF(width()-fontMetrics.width(" "+bandText+" "),2,fontMetrics.width(" "+bandText+"
    // "),fontMetrics.height()));
    // bandNameRects.push_back(QRectF(2,height()-fontMetrics.height(),fontMetrics.width(" "+bandText+"
    // "),fontMetrics.height()));
    // bandNameRects.push_back(QRectF(width()-fontMetrics.width(" "+bandText+"
    // "),height()-fontMetrics.height(),fontMetrics.width(" "+bandText+" "),fontMetrics.height()));
    ...
    for (int i = 0; i < bandNameRects.count(); i++) {
      ...
      if ((labelRect.height()) < height() and (childBaseItems().isEmpty()) and !isSelected()) {
...
void BandDesignIntf::addChildBand(BandDesignIntf *band) {
  ...
  connect(band, &QObject::destroyed, this, &BandDesignIntf::childBandDeleted);
}
...
void BandDesignIntf::setParentBandName(const QString &parentBandName) {
  ...
  if (itemMode() & DesignMode and !m_parentBandName.isEmpty()) {
    if ((parentBand() == 0) or (parentBand()->objectName() != parentBandName)) setParentBand(findParentBand());
  }
}
...
int BandDesignIntf::maxChildIndex(QSet<BandDesignIntf::BandsType> ignoredBands) const {
  ...
  foreach (BandDesignIntf *childBand, childBands()) {
    if (!ignoredBands.contains(childBand->bandType()) and childBand->bandIndex() > bandIndex()) {
...
int BandDesignIntf::minChildIndex(BandDesignIntf::BandsType bandType) {
  int curIndex = bandIndex();
  foreach (BandDesignIntf *childBand, childBands()) {
    if (curIndex > childBand->bandIndex() and (childBand->bandType() > bandType)) {
...
bool BandDesignIntf::canBeSplitted(int height) const {
  if (isSplittable()) {
    foreach (QGraphicsItem *qgItem, childItems()) {
      ...
      if (item)
        if ((item->minHeight() > height) and (item->minHeight() > (this->height() - height))) return false;
    }
  }
  return isSplittable();
}
...
bool BandDesignIntf::isEmpty() const {
  foreach (QGraphicsItem *qgItem, childItems()) {
    BaseDesignIntf *item = dynamic_cast<BaseDesignIntf *>(qgItem);
    if ((item) and (!item->isEmpty())) return false;
  }
  return true;
}
...
void BandDesignIntf::checkEmptyTable() {
  ...
  foreach (QGraphicsItem *qgItem, childItems()) {
    BaseDesignIntf *item = dynamic_cast<BaseDesignIntf *>(qgItem);
    if (item and !item->isEmpty()) isEmpty = false;
    if (!item) allItemsIsText = false;
  }
  if (isEmpty and allItemsIsText) {
    foreach (QGraphicsItem *qgItem, childItems()) {
...
void BandDesignIntf::setColumnsCount(int value) {
  if (m_columnsCount != value and value > 0) {
...
BaseDesignIntf *BandDesignIntf::cloneUpperPart(int height, QObject *owner, QGraphicsItem *parent) {
  ...
  foreach (QGraphicsItem *qgItem, childItems()) {
    ...
    if (item) {
      ...
      } else if ((item->geometry().top() < height) and (item->geometry().bottom() > height)) {
...
void BandDesignIntf::arrangeSubItems(RenderPass pass, DataSourceManager *dataManager, ArrangeType type) {
  ...
  if (needArrage) {
    ...
    for (int i = 0; i < m_bandItems.count(); i++)
      for (int j = i; j < m_bandItems.count(); j++) {
        if ((i != j) and (m_bandItems[i]->m_item->collidesWithItem(m_bandItems[j]->m_item))) {
  ...
  if (needArrage or pass == FirstPass) {
...
qreal BandDesignIntf::findMaxBottom() {
  ...
  foreach (QGraphicsItem *item, childItems()) {
    ...
    if (subItem)
      if (subItem->isVisible() and (subItem->geometry().bottom() > maxBottom)) maxBottom = subItem->geometry().bottom();
  }
  return maxBottom;
}
...
BandDesignIntf *BandDesignIntf::findParentBand() {
  if (parent() and (!dynamic_cast<BaseDesignIntf *>(parent())->isLoading())) {
    foreach (QObject *item, parent()->children()) {
      BandDesignIntf *band = dynamic_cast<BandDesignIntf *>(item);
      if (band and (band->objectName().compare(parentBandName(), Qt::CaseInsensitive) == 0)) return band;
    }
  }
  return 0;
}
...
void BandDesignIntf::geometryChangedEvent(QRectF, QRectF) {
  if (((itemMode() & DesignMode) or (itemMode() & EditMode)) and parentItem()) {
...
QVariant BandDesignIntf::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant &value) {
  if ((change == ItemPositionChange) and ((itemMode() & DesignMode) or (itemMode() & EditMode))) {
    if (m_bandMarker) {
  ...
  if (change == ItemChildAddedChange or change == ItemChildRemovedChange) {
    update(rect());
  }
...
void BandDesignIntf::initMode(ItemMode mode) {
  ...
  if ((mode == PreviewMode) or (mode == PrintMode)) {
...
void BandDesignIntf::setStartFromNewPage(bool startWithNewPage) { m_startFromNewPage = startWithNewPage; }

bool BandDesignIntf::startNewPage() const { return m_startNewPage; }

void BandDesignIntf::setStartNewPage(bool startNewPage) { m_startNewPage = startNewPage; }
...
BandDesignIntf *BandDesignIntf::bandHeader() {
  foreach (BandDesignIntf *band, childBands()) {
    if (band->isHeader() and !band->isGroupHeader()) return band;
  }
  return 0;
}
...
void BandDesignIntf::updateItemSize(DataSourceManager *dataManager, RenderPass pass, int maxHeight) {
  ...
  if (autoHeight()) {
    // if keepBottomSpace() and height()<findMaxBottom()
...
DataBandDesignIntf::DataBandDesignIntf(BandDesignIntf::BandsType bandType, QString xmlTypeName, QObject *owner,
