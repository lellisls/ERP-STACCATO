...
namespace {
LimeReport::ObjectPropItem *createLocationPropItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects,
...
bool registred = LimeReport::ObjectPropFactory::instance().registerCreator(
    LimeReport::APropIdent("itemLocation", "LimeReport::ItemDesignIntf"), QObject::tr("itemLocation"),
    createLocationPropItem);
}

LimeReport::ItemLocationPropItem::ItemLocationPropItem(QObject *object, ObjectsList *objects, const QString &name,
...
void LimeReport::ItemLocationPropItem::setModelData(QWidget *propertyEditor, QAbstractItemModel *model,
                                                    const QModelIndex &index) {
  object()->setProperty(propertyName().toLatin1(),
                        stringToLocation(qobject_cast<ComboBoxEditor *>(propertyEditor)->text()));
...
void LimeReport::ItemLocationPropItem::slotLocationChanged(const QString &text) {
  if (locationToString(object()->property(propertyName().toLatin1()).toInt()) != text) {
    ...
    dynamic_cast<ComboBoxEditor *>(sender())->setTextValue(
        locationToString(object()->property(propertyName().toLatin1()).toInt()));
  }
}
