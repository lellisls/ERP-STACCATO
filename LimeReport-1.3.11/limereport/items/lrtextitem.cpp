...
TextItem::~TextItem() { delete m_text; }

int TextItem::fakeMarginSize() { return marginSize() + 5; }
...
bool TextItem::canBeSplitted(int height) const { return height > (m_text->begin().layout()->lineAt(0).height()); }
...
QWidget *TextItem::defaultEditor() {
  QSettings *l_settings = (page()->settings() != 0) ? page()->settings() : (page()->reportEditor() != 0)
                                                                               ? page()->reportEditor()->settings()
                                                                               : 0;
