...
CREATE TABLE IF NOT EXISTS `mydb`.`conta_a_pagar_has_pagamento` (
  ...
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00' ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`idPagamento`, `idCompra`, `idLoja`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_compras` (`Compra` INT, `idVenda` INT, `Fornecedor` INT, `Produtos` INT, `Preço` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_faturamento` (`idCompra` INT, `idVenda` INT, `Fornecedor` INT, `Produtos` INT, `Preço` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fornecedor_logistica2`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fornecedor_logistica2` (`fornecedor` INT, `COLET` INT, `RECEB` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_validacao` (`idVenda` INT, `soma` INT, `total` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_venda`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda` (`Código` INT, `Vendedor` INT, `Cliente` INT, `Data` INT, `Total R$` INT, `Status` INT, `statusEntrega` INT, `Dias restantes` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_produto_pendente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produto_pendente` (`status` INT, `idVendaProduto` INT, `fornecedor` INT, `idVenda` INT, `idProduto` INT, `produto` INT, `colecao` INT, `formComercial` INT, `quant` INT, `un` INT, `Un2` INT, `Caixas` INT, `kgcx` INT, `codComercial` INT, `codBarras` INT, `custo` INT, `idCompra` INT, `Mês` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_vendas_mes`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_vendas_mes` (`idUsuario` INT, `Data` INT, `Total` INT);
...
VIEW `mydb`.`view_compras` AS
    SELECT 
        `mydb`.`pedido_fornecedor_has_produto`.`idCompra` AS `Compra`,
        GROUP_CONCAT(`mydb`.`pedido_fornecedor_has_produto`.`idVenda`
            SEPARATOR ',') AS `idVenda`,
        `mydb`.`pedido_fornecedor_has_produto`.`fornecedor` AS `Fornecedor`,
        COUNT(`mydb`.`pedido_fornecedor_has_produto`.`idProduto`) AS `Produtos`,
        SUM(`mydb`.`pedido_fornecedor_has_produto`.`preco`) AS `Preço`
    FROM
...
-- -----------------------------------------------------
-- View `mydb`.`view_fornecedor_logistica2`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `mydb`.`view_fornecedor_logistica2` ;
DROP TABLE IF EXISTS `mydb`.`view_fornecedor_logistica2`;
USE `mydb`;
CREATE 
     OR REPLACE ALGORITHM = UNDEFINED 
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `mydb`.`view_fornecedor_logistica2` AS
    SELECT 
        `mydb`.`estoque`.`fornecedor` AS `fornecedor`,
        SUM((CASE
            WHEN (`mydb`.`estoque`.`status` = 'EM COLETA') THEN 1
            ELSE 0
        END)) AS `COLET`,
        SUM((CASE
            WHEN (`mydb`.`estoque`.`status` = 'EM RECEBIMENTO') THEN 1
            ELSE 0
        END)) AS `RECEB`
    FROM
        `mydb`.`estoque`
    WHERE
        ((`mydb`.`estoque`.`status` = 'EM COLETA')
            OR (`mydb`.`estoque`.`status` = 'EM RECEBIMENTO'))
    GROUP BY `mydb`.`estoque`.`fornecedor`;
...
VIEW `mydb`.`view_produtos_pendentes` AS
    ...
    FROM
        `mydb`.`venda_has_produto` `vp`
            LEFT JOIN
        `mydb`.`venda` `v` ON `v`.`idVenda` = `vp`.`idVenda`
            LEFT JOIN
        `mydb`.`produto` `p` ON `p`.`idProduto` = `vp`.`idProduto`
...
VIEW `mydb`.`view_validacao` AS
    SELECT 
        `mydb`.`venda`.`idVenda` AS `idVenda`,
        ((`mydb`.`venda`.`subTotalLiq` + `mydb`.`venda`.`frete`) - `mydb`.`venda`.`descontoReais`) AS `soma`,
        `mydb`.`venda`.`total` AS `total`
    FROM
        `mydb`.`venda`;
...
VIEW `mydb`.`view_produto_pendente` AS
    SELECT 
        ...
        MONTHNAME(`mydb`.`venda`.`data`) AS `Mês`
    FROM
        ((`mydb`.`venda_has_produto` `v`
        LEFT JOIN `mydb`.`produto` `p` ON ((`v`.`idProduto` = `p`.`idProduto`)))
        LEFT JOIN `mydb`.`venda` ON ((`mydb`.`venda`.`idVenda` = `v`.`idVenda`)));
...
-- -----------------------------------------------------
-- View `mydb`.`view_vendas_mes`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `mydb`.`view_vendas_mes` ;
DROP TABLE IF EXISTS `mydb`.`view_vendas_mes`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_vendas_mes` AS select `mydb`.`venda`.`idUsuario` AS `idUsuario`,date_format(`mydb`.`venda`.`data`,'%m-%Y') AS `Data`,sum((`mydb`.`venda`.`total` - `mydb`.`venda`.`frete`)) AS `Total` from `mydb`.`venda` group by `mydb`.`venda`.`idUsuario`,month(`mydb`.`venda`.`data`);
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`profissional` (`idProfissional`, `pfpj`, `nome_razao`, `nomeFantasia`, `cpf`, `cnpj`, `inscEstadual`, `contatoNome`, `contatoCPF`, `contatoApelido`, `contatoRG`, `tel`, `telCel`, `telCom`, `idNextel`, `nextel`, `email`, `tipoProf`, `banco`, `agencia`, `cc`, `nomeBanco`, `cpfBanco`, `desativado`, `created`, `lastUpdated`) VALUES (1, NULL, 'Não há', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, DEFAULT, DEFAULT, DEFAULT);
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`cliente` (`idCliente`, `pfpj`, `nome_razao`, `nomeFantasia`, `cpf`, `cnpj`, `dataNasc`, `inscEstadual`, `contatoNome`, `contatoCPF`, `contatoApelido`, `contatoRG`, `tel`, `telCel`, `telCom`, `idNextel`, `nextel`, `email`, `idUsuarioRel`, `idCadastroRel`, `idProfissionalRel`, `credito`, `incompleto`, `desativado`, `created`, `lastUpdated`) VALUES (1, 'PJ', 'Eduardo Pinto de Oliveira', NULL, NULL, '11.111.111/1111-11', NULL, 'ISENTO', NULL, NULL, NULL, NULL, '(11)4153-0948', NULL, NULL, NULL, NULL, 'edu.oliv@globo.com', NULL, NULL, NULL, NULL, 0, DEFAULT, DEFAULT, DEFAULT);
