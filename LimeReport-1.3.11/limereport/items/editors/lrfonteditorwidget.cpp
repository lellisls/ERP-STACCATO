...
void FontEditorWidget::initEditor() {
  ...
  connect(m_fontSizeEditor, qOverload<const QString &>(&QComboBox::currentIndexChanged), this,
          &FontEditorWidget::slotFontSizeChanged);
  ...
  if (reportEditor()) {
    connect(reportEditor(), &ReportDesignWidget::itemPropertyChanged, this, &FontEditorWidget::slotPropertyChanged);
  }
}
...
void FontEditorWidget::slotFontChanged(const QFont &font) {
  if (reportEditor() and !m_ignoreSlots) reportEditor()->setFont(font);
}

void FontEditorWidget::slotFontSizeChanged(const QString &value) {
  if (reportEditor() and !m_ignoreSlots) {
...
void FontEditorWidget::slotPropertyChanged(const QString &objectName, const QString &property, const QVariant &oldValue,
                                           const QVariant &newValue) {
  Q_UNUSED(oldValue)
  Q_UNUSED(newValue)
  if (item() and (item()->objectName() == objectName) and (property == "font")) {
