...
QRectF PreviewReportWidgetPrivate::calcPageShift() {
  ...
  QRectF pageStartPos =
      m_reportPages.at(m_currentPage - 1)->mapRectToScene(m_reportPages.at(m_currentPage - 1)->rect());
...
PageItemDesignIntf::Ptr PreviewReportWidgetPrivate::currentPage() {
  if (m_reportPages.count() > 0 and m_reportPages.count() >= m_currentPage)
...
PreviewReportWidget::PreviewReportWidget(ReportEnginePrivate *report, QWidget *parent)
    : QWidget(parent), ui(new Ui::PreviewReportWidget), d_ptr(new PreviewReportWidgetPrivate(this)) {
  ui->setupUi(this);
  ...
  connect(ui->graphicsView->verticalScrollBar(), &QAbstractSlider::valueChanged, this,
          &PreviewReportWidget::slotSliderMoved);
  connect(d_ptr->m_report, &QObject::destroyed, this, &PreviewReportWidget::reportEngineDestroyed);
  d_ptr->m_zoomer = new GraphicsViewZoomer(ui->graphicsView);
  connect(d_ptr->m_zoomer, &GraphicsViewZoomer::zoomed, this, &PreviewReportWidget::slotZoomed);
}
...
void PreviewReportWidget::firstPage() {
  ...
  if ((!d_ptr->m_reportPages.isEmpty()) and (d_ptr->m_currentPage > 1)) {
...
void PreviewReportWidget::nextPage() {
  d_ptr->m_changingPage = true;
  if ((!d_ptr->m_reportPages.isEmpty()) and (d_ptr->m_reportPages.count() > (d_ptr->m_currentPage))) {
    d_ptr->m_currentPage++;
    ui->graphicsView->ensureVisible(d_ptr->calcPageShift(), 0, 0);
    emit pageChanged(d_ptr->m_currentPage);
  }
...
void PreviewReportWidget::pageNavigatorChanged(int value) {
  ...
  d_ptr->m_changingPage = true;
  if ((!d_ptr->m_reportPages.isEmpty()) and (d_ptr->m_reportPages.count() >= value) and value > 0) {
