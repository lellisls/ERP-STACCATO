...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_faturamento` (`fornecedor` INT, `COUNT(idProduto)` INT, `SUM(preco)` INT, `status` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produto_pendente` (`fornecedor` INT, `idVenda` INT, `idProduto` INT, `produto` INT, `colecao` INT, `formComercial` INT, `quant` INT, `un` INT, `Un2` INT, `Caixas` INT, `kgcx` INT, `codComercial` INT, `codBarras` INT, `custo` INT, `idCompra` INT, `status` INT, `estoque` INT);
...
VIEW `mydb`.`view_estoque` AS
    ...
    FROM
        `mydb`.`estoque` `e`
            LEFT JOIN
        `mydb`.`produto` `p` ON `e`.`idProduto` = `p`.`idProduto`
...
VIEW `mydb`.`view_faturamento` AS
    SELECT 
        `p`.`fornecedor` AS `fornecedor`,
        COUNT(`p`.`idProduto`) AS `COUNT(idProduto)`,
        SUM(`p`.`preco`) AS `SUM(preco)`,
        `p`.`status` AS `status`
    FROM
        `pedido_fornecedor_has_produto` `p`
    WHERE
...
VIEW `mydb`.`view_produto_pendente` AS
    ...
    FROM
        `mydb`.`venda_has_produto` `v`
            LEFT JOIN
        `mydb`.`estoque` `e` ON `v`.`idProduto` = `e`.`idProduto`
            LEFT JOIN
        `mydb`.`produto` `p` ON `v`.`idProduto` = `p`.`idProduto`;
