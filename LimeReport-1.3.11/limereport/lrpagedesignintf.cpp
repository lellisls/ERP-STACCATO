...
PageDesignIntf::PageDesignIntf(QObject *parent)
    : QGraphicsScene(parent), m_pageSize(A4), m_orientation(Portrait), m_pageItem(0), m_insertMode(false),
      m_itemInsertRect(0), m_itemMode(DesignMode), m_cutterBorder(0), m_currentCommand(-1), m_changeSizeMode(false),
      m_changePosMode(false), m_changePosOrSizeMode(false), m_executingCommand(false), m_hasHanges(false),
      m_isLoading(false), m_executingGroupCommand(false), m_settings(0), m_selectionRect(0),
      // m_verticalGridStep(1*Const::mmFACTOR),
      // m_horizontalGridStep(1*Const::mmFACTOR)
      m_verticalGridStep(2), m_horizontalGridStep(2), m_updating(false), m_currentObjectIndex(1),
      m_multiSelectStarted(false), m_movedItem(0), m_joinItem(0), m_magneticMovement(false) {
  m_reportEditor = dynamic_cast<ReportEnginePrivate *>(parent);
  updatePageRect();
  connect(this, &QGraphicsScene::selectionChanged, this, &PageDesignIntf::slotSelectionChanged);
...
void PageDesignIntf::updatePageRect() {
  if (m_pageItem.isNull()) {
    ...
    connect(m_pageItem.data(), &BaseDesignIntf::itemSelected, this, &PageDesignIntf::itemSelected);
    connect(m_pageItem.data(), &BaseDesignIntf::geometryChanged, this, &PageDesignIntf::slotPageGeomertyChanged);
    connect(m_pageItem.data(), &BaseDesignIntf::objectLoaded, this, &PageDesignIntf::slotPageItemLoaded);
  }
...
void PageDesignIntf::keyPressEvent(QKeyEvent *event) {
  if (event->modifiers() == Qt::NoModifier or
      (event->key() != Qt::Key_Left and event->key() != Qt::Key_Right and event->key() != Qt::Key_Up and
       event->key() != Qt::Key_Down and !m_changePosMode and !m_changeSizeMode)) {
    QGraphicsScene::keyPressEvent(event);
    return;
  }

  if ((event->modifiers() & Qt::ControlModifier) and (!m_changeSizeMode)) {
    if (!m_changePosMode) {
  ...
  if ((event->modifiers() & Qt::ShiftModifier) and (!m_changePosMode)) {
    if (!m_changeSizeMode) {
  ...
  if ((event->modifiers() & Qt::ControlModifier) and m_changePosMode and (!(event->modifiers() & Qt::ShiftModifier))) {
  ...
  if ((event->modifiers() & Qt::ShiftModifier) and m_changeSizeMode and (!(event->modifiers() & Qt::ControlModifier))) {
    foreach (QGraphicsItem *item, selectedItems()) {
      if (dynamic_cast<BaseDesignIntf *>(item)) {
        switch (event->key()) {
        case Qt::Key_Right:
...
void PageDesignIntf::keyReleaseEvent(QKeyEvent *event) {
  if ((event->key() == Qt::Key_Control) and m_changePosMode) {
    checkSizeOrPosChanges();
  }

  if ((event->key() == Qt::Key_Shift) and m_changeSizeMode) {
    checkSizeOrPosChanges();
  }
...
void PageDesignIntf::setPageItem(PageItemDesignIntf::Ptr pageItem) {
  ...
  setSceneRect(pageItem->rect().adjusted(-10 * Const::mmFACTOR, -10 * Const::mmFACTOR, 10 * Const::mmFACTOR,
                                         10 * Const::mmFACTOR));
  addItem(m_pageItem.data());
...
void PageDesignIntf::setPageItems(QList<PageItemDesignIntf::Ptr> pages) {
  ...
  setSceneRect(QRectF(0, 0, curWidth, curHeight)
                   .adjusted(-10 * Const::mmFACTOR, -10 * Const::mmFACTOR, 10 * Const::mmFACTOR, 10 * Const::mmFACTOR));
}
...
void PageDesignIntf::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (event->buttons() & Qt::LeftButton and m_multiSelectStarted) {
    if (!m_selectionRect) {
  ...
  if ((m_insertMode) and (pageItem()->rect().contains(pageItem()->mapFromScene(event->scenePos())))) {
...
void PageDesignIntf::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
  if ((event->button() == Qt::LeftButton)) {
    if (m_joinItem and selectedItems().count() == 1) {
...
template <typename T> BaseDesignIntf *PageDesignIntf::internalAddBand(T bandType) {
  ...
  needParentBands << BandDesignIntf::SubDetailBand << BandDesignIntf::SubDetailHeader << BandDesignIntf::SubDetailFooter
                  << BandDesignIntf::GroupHeader << BandDesignIntf::GroupFooter << BandDesignIntf::DataHeader
...
BaseDesignIntf *PageDesignIntf::createReportItem(const QString &itemType, QObject *owner, BaseDesignIntf *parent) {
  return LimeReport::DesignElementsFactory::instance().objectCreator(itemType)((owner) ? owner : pageItem(),
                                                                               (parent) ? parent : pageItem());
}
...
CommandIf::Ptr PageDesignIntf::removeReportItemCommand(BaseDesignIntf *item) {
  ...
  if (band) {
    ...
    if (layout and (layout->childrenCount() == 2)) {
...
bool PageDesignIntf::saveCommand(CommandIf::Ptr command, bool runCommand) {
  if (m_executingCommand or m_isLoading) return false;
  if (runCommand) {
...
bool PageDesignIntf::isHasChanges() { return (m_commandsList.count() > 0) and m_hasHanges; }
...
bool PageDesignIntf::isExistsObjectName(const QString &objectName, QList<QGraphicsItem *> &itemsList) const {
  ...
  for (int i = 0; i < itemsList.count(); i++) {
    QObject *item = dynamic_cast<QObject *>(itemsList[i]);
    if (item)
...
void PageDesignIntf::dropEvent(QGraphicsSceneDragDropEvent *event) {
  if (event->mimeData()->hasText() and
      ((event->mimeData()->text().indexOf("field:") == 0) or (event->mimeData()->text().indexOf("variable:") == 0))) {
...
void PageDesignIntf::slotItemPropertyChanged(QString propertyName, const QVariant &oldValue, const QVariant &newValue) {
  if (!m_isLoading and m_animationList.isEmpty()) {
...
void PageDesignIntf::slotItemPropertyObjectNameChanged(const QString &oldName, const QString &newName) {
  if (oldName.compare(newName) != 0 and !m_executingCommand) {
...
void PageDesignIntf::checkSizeOrPosChanges() {
  ...
  if ((selectedItems().count() > 0) and (m_positionStamp.count() > 0)) {
  ...
  if ((selectedItems().count() > 0) and (m_geometryStamp.count() > 0)) {
    BaseDesignIntf *reportItem = dynamic_cast<BaseDesignIntf *>(selectedItems()[0]);
    if (reportItem and (m_geometryStamp[0].size != reportItem->size())) {
      sizeCommand = createChangeSizeCommand();
    }
    m_geometryStamp.clear();
  }

  if (sizeCommand and posCommand) {
...
void PageDesignIntf::animateItem(BaseDesignIntf *item) {
  if (item and (item->metaObject()->indexOfProperty("backgroundColor") > -1)) {
    ...
    connect(ani1, &QObject::destroyed, this, &PageDesignIntf::slotAnimationStoped);
  }
}
...
void PageDesignIntf::registerItem(BaseDesignIntf *item) {
  ...
  if (band) {
    registerBand(band);
    connect(band, &BaseDesignIntf::propertyObjectNameChanged, this, &PageDesignIntf::slotItemPropertyObjectNameChanged);
    emit bandAdded(this, band);
  } else {
    connect(item, &BaseDesignIntf::propertyChanged, this, &PageDesignIntf::slotItemPropertyChanged);
    connect(item, &BaseDesignIntf::propertyObjectNameChanged, this, &PageDesignIntf::slotItemPropertyObjectNameChanged);
...
void PageDesignIntf::registerBand(BandDesignIntf *band) {
  if (pageItem() and !pageItem()->isBandRegistred(band)) {
...
void PageDesignIntf::saveChangeProppertyCommand(const QString &objectName, const QString &propertyName,
                                                const QVariant &oldPropertyValue, const QVariant &newPropertyValue) {
  if (!m_executingCommand) {
    CommandIf::Ptr command;
    if (propertyName.compare("ItemAlign", Qt::CaseInsensitive) == 0) {
      command =
          PropertyItemAlignChangedCommand::create(this, objectName, BaseDesignIntf::ItemAlign(oldPropertyValue.toInt()),
...
void PageDesignIntf::changeSelectedGroupProperty(const QString &name, const QVariant &value) {
  if ((selectedItems().count() > 0) and m_firstSelectedItem) {
    CommandGroup::Ptr cm = CommandGroup::create();
    m_executingCommand = true;
    foreach (QGraphicsItem *item, selectedItems()) {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem) {
        ...
        if (oldValue.isValid()) {
          ...
          CommandIf::Ptr command =
              PropertyChangedCommand::create(this, bdItem->objectName(), "alignment", oldValue, value);
          cm->addCommand(command, false);
        }
      }
    }
...
void PageDesignIntf::bringToFront() {
  foreach (QGraphicsItem *item, selectedItems()) {
    ...
    if (bdItem) saveChangeProppertyCommand(bdItem->objectName(), "zOrder", bdItem->zValue(), zOrder);
  ...
  if ((selectedItems().count() > 0) and m_firstSelectedItem) {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem) {
        ...
        CommandIf::Ptr command =
            PropertyChangedCommand::create(this, bdItem->objectName(), "geometry", oldGeometry, bdItem->geometry());
        cm->addCommand(command, false);
      }
    }
    saveCommand(cm, false);
  ...
  if ((selectedItems().count() > 0) and m_firstSelectedItem) {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem) {
        QRectF oldGeometry = bdItem->geometry();
        bdItem->setPos(
            QPoint((m_firstSelectedItem->geometry().right() - m_firstSelectedItem->width() / 2) - bdItem->width() / 2,
                   bdItem->pos().y()));
        CommandIf::Ptr command =
            PropertyChangedCommand::create(this, bdItem->objectName(), "geometry", oldGeometry, bdItem->geometry());
        cm->addCommand(command, false);
  ...
  if ((selectedItems().count() > 0) and m_firstSelectedItem) {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem) {
        QRectF oldGeometry = bdItem->geometry();
        bdItem->setPos(
            QPoint(bdItem->pos().x(), (m_firstSelectedItem->geometry().bottom() - m_firstSelectedItem->height() / 2) -
                                          bdItem->height() / 2));
        CommandIf::Ptr command =
            PropertyChangedCommand::create(this, bdItem->objectName(), "geometry", oldGeometry, bdItem->geometry());
        cm->addCommand(command, false);
  ...
  if (!si.isEmpty()) {
    ...
    for (; it != si.end(); ++it) {
      if ((*it)->parentItem() != elementsParent) {
        QMessageBox::information(0, QObject::tr("Attention!"),
...
HorizontalLayout *PageDesignIntf::internalAddHLayout() {
  if (m_firstSelectedItem and (selectedItems().count() > 1)) {
...
BaseDesignIntf *PageDesignIntf::reportItemByName(const QString &name) {
  ...
  foreach (QGraphicsItem *item, items()) {
    ...
    if (bd and (bd->objectName().compare(name, Qt::CaseInsensitive) == 0)) return bd;
  }
...
QList<BaseDesignIntf *> PageDesignIntf::reportItemsByName(const QString &name) {
  ...
  foreach (QGraphicsItem *item, items()) {
    BaseDesignIntf *bd = dynamic_cast<BaseDesignIntf *>(item);
    if (bd and (bd->objectName().compare(name, Qt::CaseInsensitive) == 0)) result.append(bd);
  }
  return result;
}
...
CommandIf::Ptr DeleteLayoutCommand::create(PageDesignIntf *page, LayoutDesignIntf *item) {
  ...
  command->setPage(page);
  command->setItem(item);
  foreach (BaseDesignIntf *item, item->childBaseItems()) { command->m_childItems.append(item->objectName()); }
  return CommandIf::Ptr(command);
}
...
bool PasteCommand::doIt() {
  ...
  if (reader->first() and reader->itemType() == "Object") {
    page()->beginUpdate();
    insertItem(reader);
...
bool PosChangedCommand::doIt() {
  for (int i = 0; i < m_newPos.count(); i++) {
    ...
    if (reportItem and (reportItem->pos() != m_newPos[i].pos)) reportItem->setPos(m_newPos[i].pos);
  }
...
void PosChangedCommand::undoIt() {
  for (int i = 0; i < m_oldPos.count(); i++) {
    ...
    if (reportItem and (reportItem->pos() != m_oldPos[i].pos)) reportItem->setPos(m_oldPos[i].pos);
  }
}
...
bool SizeChangedCommand::doIt() {
  for (int i = 0; i < m_newSize.count(); i++) {
    ...
    if (reportItem and (reportItem->size() != m_newSize[i].size)) reportItem->setSize(m_newSize[i].size);
  }
...
void SizeChangedCommand::undoIt() {
  for (int i = 0; i < m_oldSize.count(); i++) {
    ...
    if (reportItem and (reportItem->size() != m_oldSize[i].size)) reportItem->setSize(m_oldSize[i].size);
  }
}
...
bool PropertyChangedCommand::doIt() {
  ...
  if (reportItem and (reportItem->property(m_propertyName.toLatin1()) != m_newValue)) {
...
void PropertyChangedCommand::undoIt() {
  ...
  if (reportItem and (reportItem->property(m_propertyName.toLatin1()) != m_oldValue)) {
...
bool InsertBandCommand::doIt() {
  if (!m_parentBandName.isEmpty() and page()->reportItemByName(m_parentBandName))
...
CommandIf::Ptr PropertyObjectNameChangedCommand::create(PageDesignIntf *page, const QString &oldValue,
...
bool Projection::intersect(Projection projection) {
  if ((projection.start() >= start() and projection.start() <= end()) or
      (projection.end() >= start() and projection.end() <= end()) or
      (projection.start() <= start() and projection.end() >= end()))
    return true;
  return false;
}
...
bool ItemProjections::intersect(QRectF rect) {
  ...
  if (m_xProjection.intersect(xProjection) and m_yProjection.intersect(yProjection)) return true;
  return false;
}
...
qreal lineLength(qreal start, qreal end, Projection p) {
  qreal result = 0;
  if (start >= p.start() and end <= p.end()) result = end - start;
  if (start >= p.start() and start <= p.end())
    result = p.end() - start;
  else if (end >= p.start() and end <= p.end())
    result = end - p.start();
  else if (start <= p.start() and end >= p.end())
