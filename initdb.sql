...
CREATE TABLE IF NOT EXISTS `mydb`.`conta_representacao` (
  `idContaRepresentacao` INT(11) NOT NULL AUTO_INCREMENT,
  `idVenda` VARCHAR(20) NOT NULL,
  `dataEmissao` DATETIME NOT NULL,
  `dataPagamento` DATETIME NULL DEFAULT NULL,
  `pago` VARCHAR(5) NOT NULL DEFAULT 'NÃO',
  `formaPagamento` VARCHAR(45) NULL DEFAULT NULL,
  `idProduto` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`idContaRepresentacao`, `idVenda`))
ENGINE = InnoDB;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fornecedor_compra` (`fornecedor` INT, `PEND` INT, `INIC` INT, `COMP` INT, `FAT` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fornecedor_logistica`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fornecedor_logistica` (`fornecedor` INT, `COUNT(fornecedor)` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_orcamento`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_orcamento` (`Código` INT, `Vendedor` INT, `Cliente` INT, `Total` INT, `Data de emissão` INT, `Status` INT, `Dias restantes` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_pendente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_pendente` (`fornecedor` INT, `produto` INT, `formComercial` INT, `SUM(v.quant)` INT, `un` INT, `codComercial` INT, `idCompra` INT, `status` INT, `quant` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_produtos_pendentes`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produtos_pendentes` (`Fornecedor` INT, `Descrição` INT, `Form` INT, `Quant` INT, `Un` INT, `Cód Com` INT, `idCompra` INT, `Status` INT, `Estoque` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_venda`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda` (`Código` INT, `Loja` INT, `Vendedor` INT, `Cliente` INT, `Endereço` INT, `Profissional` INT, `Data` INT, `Bruto` INT, `Líquido` INT, `Frete` INT, `Total R$` INT, `Status` INT, `Dias restantes` INT);
...
VIEW `view_compras` AS
    SELECT 
        `pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,
        `pedido_fornecedor_has_produto`.`idCompra` AS `idCompra`,
        COUNT(`pedido_fornecedor_has_produto`.`idProduto`) AS `COUNT(idProduto)`,
        SUM(`pedido_fornecedor_has_produto`.`preco`) AS `SUM(preco)`,
        `pedido_fornecedor_has_produto`.`status` AS `status`
    FROM
        `pedido_fornecedor_has_produto`
    WHERE
        (`pedido_fornecedor_has_produto`.`status` = 'EM COMPRA')
    GROUP BY `pedido_fornecedor_has_produto`.`idCompra`;
...
VIEW `view_estoque` AS
    ...
    FROM
        (`estoque` `e`
        LEFT JOIN `produto` `p` ON ((`e`.`codComercial` = `p`.`codComercial`)))
...
VIEW `view_faturamento` AS
    ...
    FROM
        `pedido_fornecedor_has_produto`
    WHERE
        (`pedido_fornecedor_has_produto`.`status` = 'EM FATURAMENTO')
    GROUP BY `pedido_fornecedor_has_produto`.`idCompra`;
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `view_fornecedor_compra` AS
    SELECT 
        `pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,
        SUM((CASE
            WHEN (`pedido_fornecedor_has_produto`.`status` = 'PENDENTE') THEN 1
            ELSE 0
        END)) AS `PEND`,
        SUM((CASE
            WHEN (`pedido_fornecedor_has_produto`.`status` = 'INICIADO') THEN 1
            ELSE 0
        END)) AS `INIC`,
        SUM((CASE
            WHEN (`pedido_fornecedor_has_produto`.`status` = 'EM COMPRA') THEN 1
            ELSE 0
        END)) AS `COMP`,
        SUM((CASE
            WHEN (`pedido_fornecedor_has_produto`.`status` = 'EM FATURAMENTO') THEN 1
            ELSE 0
        END)) AS `FAT`
    FROM
        `pedido_fornecedor_has_produto`
    WHERE
        ((`pedido_fornecedor_has_produto`.`status` = 'PENDENTE')
            OR (`pedido_fornecedor_has_produto`.`status` = 'INICIADO')
            OR (`pedido_fornecedor_has_produto`.`status` = 'EM COMPRA')
            OR (`pedido_fornecedor_has_produto`.`status` = 'EM FATURAMENTO'))
    GROUP BY `pedido_fornecedor_has_produto`.`fornecedor`;
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `view_fornecedor_logistica` AS
    SELECT 
        `pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,
        COUNT(`pedido_fornecedor_has_produto`.`fornecedor`) AS `COUNT(fornecedor)`
    FROM
        `pedido_fornecedor_has_produto`
    WHERE
        ((`pedido_fornecedor_has_produto`.`status` = 'EM COLETA')
            OR (`pedido_fornecedor_has_produto`.`status` = 'EM RECEBIMENTO'))
    GROUP BY `pedido_fornecedor_has_produto`.`fornecedor`;
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `view_orcamento` AS
    SELECT 
        `orcamento`.`idOrcamento` AS `Código`,
        `usuario`.`nome` AS `Vendedor`,
        `cliente`.`nome_razao` AS `Cliente`,
        `orcamento`.`total` AS `Total`,
        `orcamento`.`data` AS `Data de emissão`,
        `orcamento`.`status` AS `Status`,
        (`orcamento`.`validade` - (TO_DAYS(NOW()) - TO_DAYS(`orcamento`.`data`))) AS `Dias restantes`
    FROM
        ((`orcamento`
        LEFT JOIN `cliente` ON ((`orcamento`.`idCliente` = `cliente`.`idCliente`)))
        LEFT JOIN `usuario` ON ((`orcamento`.`idUsuario` = `usuario`.`idUsuario`)));

-- -----------------------------------------------------
-- View `mydb`.`view_pendente`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `mydb`.`view_pendente` ;
DROP TABLE IF EXISTS `mydb`.`view_pendente`;
USE `mydb`;
CREATE 
     OR REPLACE ALGORITHM = UNDEFINED 
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `view_pendente` AS
    SELECT 
        `v`.`fornecedor` AS `fornecedor`,
        `v`.`produto` AS `produto`,
        `v`.`formComercial` AS `formComercial`,
        SUM(`v`.`quant`) AS `SUM(v.quant)`,
        `v`.`un` AS `un`,
        ...
        (SELECT 
            ...
            FROM
                `estoque`
            WHERE
                (`estoque`.`codComercial` = `v`.`codComercial`)) AS `quant`
    FROM
        (`venda_has_produto` `v`
        LEFT JOIN `estoque` `e` ON ((`v`.`idProduto` = `e`.`idProduto`)))
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `view_produtos_pendentes` AS
    SELECT 
        `v`.`fornecedor` AS `Fornecedor`,
        `v`.`produto` AS `Descrição`,
        `v`.`formComercial` AS `Form`,
        SUM(`v`.`quant`) AS `Quant`,
        `v`.`un` AS `Un`,
        `v`.`codComercial` AS `Cód Com`,
        `v`.`idCompra` AS `idCompra`,
        `v`.`status` AS `Status`,
        (SELECT 
                SUM(`estoque`.`quant`)
            FROM
                `estoque`
            WHERE
                (`estoque`.`codComercial` = `v`.`codComercial`)) AS `Estoque`
    FROM
        (`venda_has_produto` `v`
        LEFT JOIN `estoque` `e` ON ((`v`.`idProduto` = `e`.`idProduto`)))
    GROUP BY `v`.`status` , `v`.`codComercial`;
...
VIEW `view_venda` AS
    SELECT 
        ...
        `v`.`data` AS `Data`,
        `v`.`subTotalBru` AS `Bruto`,
        `v`.`subTotalLiq` AS `Líquido`,
        `v`.`frete` AS `Frete`,
        `v`.`total` AS `Total R$`,
        `v`.`status` AS `Status`,
    ...
    FROM
        (((((`venda` `v`
        LEFT JOIN `loja` `l` ON ((`v`.`idLoja` = `l`.`idLoja`)))
        LEFT JOIN `usuario` `u` ON ((`v`.`idUsuario` = `u`.`idUsuario`)))
        LEFT JOIN `cliente_has_endereco` `e` ON ((`v`.`idEnderecoEntrega` = `e`.`idEndereco`)))
        LEFT JOIN `profissional` `p` ON ((`v`.`idProfissional` = `p`.`idProfissional`)))
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`loja` (`idLoja`, `descricao`, `nomeFantasia`, `razaoSocial`, `tel`, `tel2`, `inscEstadual`, `sigla`, `cnpj`, `codUF`, `porcentagemFrete`, `valorMinimoFrete`, `desativado`) VALUES (1, 'Alphaville', 'Staccato', 'STACCATO REVESTIMENTOS COMÉRCIO E REPRESENTAÇÃO LTDA', '(11)4191-9816', NULL, '206.257.840.114', 'ALPH', '09.375.013/0001-10', 35, 4, 80, DEFAULT);
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`profissional` (`idProfissional`, `pfpj`, `nome_razao`, `nomeFantasia`, `cpf`, `cnpj`, `rg`, `inscEstadual`, `contatoNome`, `contatoCPF`, `contatoApelido`, `contatoRG`, `tel`, `telCel`, `telCom`, `idNextel`, `nextel`, `email`, `tipoProf`, `banco`, `agencia`, `cc`, `nomeBanco`, `cpfBanco`, `desativado`) VALUES (1, NULL, 'Não há', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, DEFAULT);
