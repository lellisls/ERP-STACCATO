...
CREATE TABLE IF NOT EXISTS `mydb`.`orcamento_has_followup` (
  ...
  PRIMARY KEY (`idFollowup`, `idOrcamento`, `idLoja`, `idUsuario`))
ENGINE = InnoDB
AUTO_INCREMENT = 6
DEFAULT CHARACTER SET = utf8;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_coleta` (`idVendaProduto` INT, `fornecedor` INT, `numeroNFe` INT, `produto` INT, `quant` INT, `un` INT, `caixas` INT, `idVenda` INT, `ordemCompra` INT, `local` INT, `dataRealCompra` INT, `dataRealConf` INT, `dataRealFat` INT, `dataPrevColeta` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_faturamento` (`OC` INT, `Compra` INT, `Código` INT, `Fornecedor` INT, `Produtos` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda` (`Código` INT, `Vendedor` INT, `Indicou` INT, `Cliente` INT, `Data` INT, `Total R$` INT, `Status` INT, `statusEntrega` INT, `Dias restantes` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_produto_estoque`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produto_estoque` (`selecionado` INT, `status` INT, `idVendaProduto` INT, `idNfeSaida` INT, `fornecedor` INT, `idVenda` INT, `idLoja` INT, `item` INT, `idProduto` INT, `produto` INT, `obs` INT, `prcUnitario` INT, `caixas` INT, `quant` INT, `un` INT, `unCaixa` INT, `codComercial` INT, `formComercial` INT, `parcial` INT, `desconto` INT, `parcialDesc` INT, `descGlobal` INT, `total` INT, `idCompra` INT, `dataPrevCompra` INT, `dataRealCompra` INT, `dataPrevConf` INT, `dataRealConf` INT, `dataPrevFat` INT, `dataRealFat` INT, `dataPrevColeta` INT, `dataRealColeta` INT, `dataPrevReceb` INT, `dataRealReceb` INT, `dataPrevEnt` INT, `dataRealEnt` INT, `created` INT, `lastUpdated` INT, `cfop` INT, `ncm` INT, `tipoICMS` INT, `orig` INT, `cstICMS` INT, `modBC` INT, `vBC` INT, `pICMS` INT, `vICMS` INT, `modBCST` INT, `pMVAST` INT, `vBCST` INT, `pICMSST` INT, `vICMSST` INT, `cEnq` INT, `cstIPI` INT, `cstPIS` INT, `vBCPIS` INT, `pPIS` INT, `vPIS` INT, `cstCOFINS` INT, `vBCCOFINS` INT, `pCOFINS` INT, `vCOFINS` INT, `codBarras` INT);
...
VIEW `mydb`.`view_coleta` AS
    SELECT 
        `e`.`idVendaProduto` AS `idVendaProduto`,
    ...
    FROM
        ((`mydb`.`estoque` `e`
        LEFT JOIN `mydb`.`venda_has_produto` `vp` ON ((`e`.`idVendaProduto` = `vp`.`idVendaProduto`)))
        LEFT JOIN `mydb`.`produto` `p` ON ((`e`.`idProduto` = `p`.`idProduto`)))
    WHERE
        ((`e`.`status` = 'EM COLETA')
            OR (`e`.`status` = 'PRÉ-CONSUMO'))
        ...
        SUM((CASE
            WHEN
                ((`mydb`.`estoque`.`status` = 'EM COLETA')
                    OR (`mydb`.`estoque`.`status` = 'PRÉ-CONSUMO'))
            THEN
                1
            ELSE 0
        END)) AS `COLET`,
...
VIEW `mydb`.`view_nfe` AS
    ...
    FROM
        (((`mydb`.`nfe` `n`
        LEFT JOIN `mydb`.`pedido_fornecedor_has_produto` `pf` ON ((`n`.`idCompra` = `pf`.`idCompra`)))
        LEFT JOIN `mydb`.`venda` `v` ON ((`n`.`idVenda` = `v`.`idVenda`)))
...
VIEW `mydb`.`view_produto_estoque` AS
    ...
    FROM
        ...
        LEFT JOIN `mydb`.`estoque` `e` ON ((`v`.`idVendaProduto` = `e`.`idVendaProduto`)))
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`profissional` (`idProfissional`, `idLoja`, `idUsuarioRel`, `pfpj`, `nome_razao`, `nomeFantasia`, `cpf`, `cnpj`, `inscEstadual`, `contatoNome`, `contatoCPF`, `contatoApelido`, `contatoRG`, `tel`, `telCel`, `telCom`, `idNextel`, `nextel`, `email`, `tipoProf`, `banco`, `agencia`, `cc`, `nomeBanco`, `cpfBanco`, `comissao`, `desativado`, `created`, `lastUpdated`) VALUES (1, NULL, NULL, NULL, 'Não há', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, DEFAULT, DEFAULT, DEFAULT);
