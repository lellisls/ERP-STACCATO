...
ComboBoxEditor::ComboBoxEditor(QWidget *parent, bool clearable)
    : QWidget(parent), m_comboBox(new InternalComboBox(this)), m_buttonClear(0), m_settingValues(false) {
  ...
  if (clearable) {
    ...
    connect(m_buttonClear, &QAbstractButton::clicked, this, &ComboBoxEditor::slotClearButtonClicked);
  }

  connect(m_comboBox, qOverload<const QString &>(&QComboBox::currentIndexChanged), this,
          &ComboBoxEditor::slotCurrentIndexChanged);
...
bool ComboBoxEditor::eventFilter(QObject *target, QEvent *event) {
  ...
  if (target == m_comboBox) {
    if (event->type() == QEvent::FocusOut) {
      if (!m_comboBox->isPopup() or (m_buttonClear and m_buttonClear->hasFocus())) emit editingFinished();
    }
  }
