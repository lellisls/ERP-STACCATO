...
QDomNode QtRPT::getBand(BandType type, QDomElement docElem) {
  ...
  while (not n.isNull()) {
    ...
    if ((not e.isNull()) and (e.tagName() == "ReportBand")) {
...
QPen QtRPT::getPen(RptFieldObject *fieldObject) {
  QPen pen;
  if (painter.isActive()) pen = painter.pen();
  // Set border width
...
Qt::PenStyle QtRPT::getPenStyle(QString value) {
  ...
  if (value == "dashed")
    ...
    style = Qt::DotLine;
  else if (value == "dot-dash")
    style = Qt::DashDotLine;
  else if (value == "dot-dot-dash")
...
FieldType QtRPT::getFieldType(QDomElement e) {
  if (e.attribute("type", "label") == "barcode") {
    ...
    return TextImage;
  } else if (e.attribute("type", "label") == "image" or e.attribute("picture", "text") != "text") {
    return Image;
...
void QtRPT::drawFields(RptFieldObject *fieldObject, int bandTop, bool draw) {
  ...
  if (draw) fieldObject->updateHighlightingParam();
  ...
  if (fieldObject->autoHeight == 1) {
    height_ =
        fieldObject->parentBand->realHeight * koefRes_h; // band.toElement().attribute("realHeight").toInt()*koefRes_h;
  }
  ...
  if (draw) {
    if (not getDrawingFields().contains(fieldType) and fieldType != Barcode) {
      // Fill background
    ...
    if (fieldType == TextImage) { // Proccess field as ImageField
      ...
      if (not image.isNull()) {
        ...
        if (painter.isActive()) painter.drawImage(point, scaledImage);
      }
    }
    if (fieldType == Image) { // Proccess as static ImageField
      ...
      // QPixmap pixmap = QPixmap::fromImage(QImage::fromData(fieldObject->picture,
      // fieldObject->imgFormat.toLatin1().data()));
      if (fieldObject->ignoreAspectRatio == 1) {
        if (painter.isActive()) painter.drawImage(QRectF(left_, top_, width_, height_), image);
    ...
    if (fieldType == Diagram) {
      ...
      chart->setParams(fieldObject->showGrid, fieldObject->showLegend, fieldObject->showCaption,
                       fieldObject->showGraphCaption, fieldObject->showPercent, fieldObject->caption,
      ...
      if (painter.isActive()) chart->paintChart(&painter);
    }
  ...
  if (fieldType == TextRich) {
    ...
    while (block.isValid()) {
      for (auto it = block.begin(); not it.atEnd(); ++it) {
        QTextFragment currentFragment = it.fragment();
        if (not currentFragment.isValid()) continue;

        if ((currentFragment.text().contains("[") and currentFragment.text().contains("]")) or
            (currentFragment.text().contains("<") and currentFragment.text().contains(">"))) {
          ...
          if (tmpTxt.isEmpty() or tmpTxt.isNull()) tmpTxt = " ";
          if (tmpTxt.toLower().contains("<body") and tmpTxt.toLower().contains("</body>")) {
  ...
  if (fieldType == Text) { // NOT Proccess if field set as ImageField
    ...
    if (painter.isActive()) painter.setPen(pen);
    int flags = fieldObject->aligment | Qt::TextDontClip; // getAligment(e);
    if (fieldObject->textWrap == 1) flags = flags | Qt::TextWordWrap;
    if (draw) {
      if (painter.isActive()) painter.drawText(left_ + 10, top_, width_ - 15, height_, flags, txt);
      ...
      if (boundRect.height() > height_ and fieldObject->autoHeight == 1) {
...
void QtRPT::drawLines(RptFieldObject *fieldObject, int bandTop) {
  ...
  if (painter.isActive()) painter.setPen(pen);
  if (fieldType == Line) {
    if (painter.isActive()) painter.drawLine(startX, startY, endX, endY);
  }
  ...
  if (painter.isActive()) painter.setBrush(fieldObject->borderColor);
  ...
  if (fieldObject->arrowStart == 1) {
    ...
    QPointF sourceArrowP2 =
        sourcePoint + QPointF(sin(angle + Pi - Pi / 3) * arrowSize, cos(angle + Pi - Pi / 3) * arrowSize);
    if (painter.isActive()) painter.drawPolygon(QPolygonF() << line.p1() << sourceArrowP1 << sourceArrowP2);
  }
  if (fieldObject->arrowEnd == 1) {
    QPointF destArrowP1 = destPoint + QPointF(sin(angle - Pi / 3) * arrowSize, cos(angle - Pi / 3) * arrowSize);
    QPointF destArrowP2 =
        destPoint + QPointF(sin(angle - Pi + Pi / 3) * arrowSize, cos(angle - Pi + Pi / 3) * arrowSize);
    if (painter.isActive()) painter.drawPolygon(QPolygonF() << line.p2() << destArrowP1 << destArrowP2);
  }
}
...
void QtRPT::drawBandRow(RptBandObject *band, int bandTop, bool allowDraw) {
  ...
  for (int i = 0; i < band->fieldList.size(); i++) {
    if (band->fieldList.at(i)->fieldType != Line and isFieldVisible(band->fieldList.at(i))) {
...
QVariant QtRPT::processHighligthing(RptFieldObject *field, HiType type) {
  if (field->highlighting.isEmpty() or field->highlighting.isNull()) {
    switch (type) {
      case FntBold: {
    ...
    for (int i = 1; i < list.size(); i++) {
      ...
      if (list.at(i).contains("bold") and type == FntBold) {
        exp.remove("bold=");
      ...
      if (list.at(i).contains("italic") and type == FntItalic) {
        exp.remove("italic=");
        QString formulaStr = exp.insert(0, cond);
        formulaStr = sectionField(field->parentBand, formulaStr, true);
        QScriptEngine myEngine;
      ...
      if (list.at(i).contains("underline") and type == FntUnderline) {
      ...
      if (list.at(i).contains("strikeout") and type == FntStrikeout) {
      ...
      if (list.at(i).contains("fontColor") and type == FntColor) {
        exp.remove("fontColor=");
        QString formulaStr = exp.insert(1, "'");
        formulaStr = exp.insert(0, cond);
        formulaStr =
            sectionField(field->parentBand, formulaStr, true) + "':'" + colorToString(field->m_fontColor) + "'";
        QScriptEngine myEngine;
        return myEngine.evaluate(formulaStr).toString();
      }
      if (list.at(i).contains("backgroundColor") and type == BgColor) {
        ...
        formulaStr =
            sectionField(field->parentBand, formulaStr, true) + "':'" + colorToString(field->m_backgroundColor) + "'";
...
QScriptValue funcAggregate(QScriptContext *context, QScriptEngine *engine) {
  ...
  for (int i = 0; i < listOfPair.size(); i++) {
    if (listOfPair.at(i).paramName == paramName) {
      if (listIdxOfGroup.size() > 0 and self.property("showInGroup").toBool() == true) {
        for (int k = 0; k < listIdxOfGroup.size(); k++) {
          if (listIdxOfGroup.at(k) == listOfPair.at(i).lnNo) {
            total += listOfPair.at(i).paramValue.toDouble();
            count += 1;
            if (max < listOfPair.at(i).paramValue.toDouble()) max = listOfPair.at(i).paramValue.toDouble();
            if (min > listOfPair.at(i).paramValue.toDouble()) min = listOfPair.at(i).paramValue.toDouble();
          }
        }
      } else {
        if (not listOfPair.at(i).paramValue.toString().isEmpty()) {
          total += listOfPair.at(i).paramValue.toDouble();
          count += 1;
          if (max < listOfPair.at(i).paramValue.toDouble()) max = listOfPair.at(i).paramValue.toDouble();
          if (min > listOfPair.at(i).paramValue.toDouble()) min = listOfPair.at(i).paramValue.toDouble();
...
QScriptValue funcText(QScriptContext *context, QScriptEngine *engine) {
  ...
  QString paramValue;
  if (paramLanguage == "ENG") paramValue = double2MoneyENG(value);
  if (paramLanguage == "UKR") paramValue = double2MoneyUKR(value, 0);
...
QStringList QtRPT::splitValue(QString value) {
  ...
  for (int i = 0; i < value.size(); ++i) {
    if (value.at(i) != '[' and value.at(i) != ']')
      ...
      if (value.at(i) == '[') {
        if (not tmpStr.isEmpty()) res << tmpStr;
        tmpStr.clear();
        tmpStr += value.at(i);
      }
    }
...
QString QtRPT::sectionField(RptBandObject *band, QString value, bool exp, bool firstPass, QString formatString) {
  ...
  for (int i = 0; i < value.size(); ++i) {
    if (value.at(i) != '[' and value.at(i) != ']' and value.at(i) != '<' and value.at(i) != '>' and not aggregate)
      ...
      tmpStr += value.at(i);
    else if (value.at(i) != '<' and value.at(i) != '>' and aggregate)
      tmpStr += value.at(i);
    else {
      if (exp and (value.at(i) == '<' or value.at(i) == '>')) tmpStr += value.at(i);
      if (value.at(i) == ']' and not aggregate) {
        tmpStr += value.at(i);
        res << tmpStr;
        tmpStr.clear();
      }
  ...
  for (int i = 0; i < res.size(); ++i) {
    if (res.at(i).contains("[") and res.at(i).contains("]") and not res.at(i).contains("<")) {
      ...
      if (exp) { // Process highlighting and visibility
        tmp.toDouble(&ok);
        if (not ok) tmp.toFloat(&ok);
        if (not ok) tmp.toInt(&ok);
        if (not ok)
          tmpStr += "'" + tmp + "'"; // Not a number
        else
          tmpStr += tmp;
        ...
        if (firstPass) { // Process during first pass to calculate aggregate values
          ...
          for (int j = 0; j < listOfPair.size(); ++j) {
            if (listOfPair.at(j).pageReport == av.pageReport and listOfPair.at(j).lnNo == av.lnNo and
                listOfPair.at(j).paramName == av.paramName)
              founded = true;
          }
          if (not founded) listOfPair.append(av);
        }
        tmpStr += getFormattedValue(tmp, formatString); // tmp;
      }
    } else {
      if (res[i].contains("<Date>")) res[i] = res[i].replace("<Date>", processFunctions("Date").toString());
      if (res[i].contains("<Time>")) res[i] = res[i].replace("<Time>", QTime::currentTime().toString());
      if (res[i].contains("<Page>")) res[i] = res[i].replace("<Page>", QString::number(curPage));
      if (res[i].contains("<TotalPages>")) res[i] = res[i].replace("<TotalPages>", QString::number(totalPage));
      if (res[i].contains("<LineNo>")) res[i] = res[i].replace("<LineNo>", processFunctions("LineNo").toString());
      if (res[i].contains("<LineCount>"))
        res[i] = res[i].replace("<LineCount>", processFunctions("LineCount").toString());

      if (res[i].contains("<") and res[i].contains(">")) {
        QString formulaStr = res[i];
        QScriptEngine myEngine;

        QStringList tl = splitValue(formulaStr);
        for (int j = 1; j < tl.size(); ++j) {
          if (tl.at(j).contains("[") and tl.at(j).contains("]") and not tl.at(j - 1).toUpper().contains("SUM") and
              not tl.at(j - 1).toUpper().contains("AVG") and not tl.at(j - 1).toUpper().contains("COUNT") and
              not tl.at(j - 1).toUpper().contains("MAX") and not tl.at(j - 1).toUpper().contains("MIN") and
              not tl.at(j - 1).toUpper().contains("NumberToWords") and not tl.at(j - 1).toUpper().contains("Frac"))
...
void QtRPT::fillListOfValue(RptBandObject *bandObject) {
  for (int i = 0; i < bandObject->fieldList.size(); i++) {
    if (bandObject->fieldList.at(i)->fieldType == Text and isFieldVisible(bandObject->fieldList.at(i))) {
...
QVariant QtRPT::processFunctions(QString value) {
  if (value.contains("Date")) return QDate::currentDate().toString("dd.MM.yyyy");
  if (value.contains("Time")) return QTime::currentTime().toString();
  if (value.contains("Page")) return QString::number(curPage);
  if (value.contains("TotalPages")) return QString::number(totalPage);
  if (value.contains("LineNo")) {
    int recNo;
    if (not listOfGroup.isEmpty()) // group processing
      recNo = mg_recNo;
  ...
  if (value.contains("LineCount")) {
    int maxLnNo = 0;
    for (int i = 0; i < listOfPair.size(); i++) {
      if (listOfPair.at(i).pageReport == m_pageReport and listOfPair.at(i).lnNo > maxLnNo)
...
QImage QtRPT::sectionFieldImage(QString value) { return sectionValueImage(value); }
...
void QtRPT::printPDF(const QString &filePath, bool open) {
  ...
  if (open) QDesktopServices::openUrl(QUrl("file:" + filePath));
#endif
}
...
void QtRPT::printExec(bool maximum, bool direct, QString printerName) {
  ...
  if (not direct) {
    ...
    if (maximum) {
      ...
      if (not list.isEmpty()) // paranoiac safety check
...
void QtRPT::exportTo() {
  if (sender()->objectName() == "actExpToPdf") {
    QString fileName = QFileDialog::getSaveFileName(qobject_cast<QWidget *>(this->parent()), tr("Save File"), "",
                                                    tr("PDF Files (*.pdf)"));
    if (fileName.isEmpty() or fileName.isNull()) return;
...
bool QtRPT::eventFilter(QObject *obj, QEvent *e) {
  if (obj == pr and e->type() == QEvent::Show) {
    for (int i = 0; i < lst.size(); i++) {
      if (lst.at(i)->text().contains("Previous page", Qt::CaseInsensitive)) lst.at(i)->trigger();
    }
...
bool QtRPT::allowPrintPage(bool draw, int curPage_) {
  if (draw) {
    if (curPage_ < fromPage) draw = false;
    if ((toPage != 0) and (curPage_ > toPage)) draw = false;
    return draw;
  } else
    return false;
}
...
void QtRPT::newPage(QPrinter *printer, int &y, bool draw, bool newReportPage) {
  ...
  if (m_printMode != QtRPT::Html) y = 0;
  if (newReportPage) processRTitle(y, draw);
  processPHeader(y, draw);
  processPFooter(draw);
}

void QtRPT::setBackgroundImage(QPixmap &image) { m_backgroundImage = &image; }

void QtRPT::setBackgroundImage(QPixmap image) { m_backgroundImage = &image; }

void QtRPT::drawBackground(QPainter &painter) {
  if (painter.isActive()) painter.setBackgroundMode(Qt::TransparentMode);
  if (m_backgroundImage != 0) {
    if (painter.isActive())
      painter.drawPixmap(-ml * koefRes_w, -mt * koefRes_h, pw * koefRes_w, ph * koefRes_h, *m_backgroundImage);
  }
}
...
void QtRPT::prcessGroupHeader(QPrinter *printer, int &y, bool draw, int pageReport) {
  ...
  if (pageList.at(m_pageReport)->getBand(DataGroupHeader) == 0) {
    ...
    if (listOfPair.size() > 0) {
      ...
      if (bandObj != 0 and bandObj->showInGroup == 0) processMHeader(y, draw);

      int grpNo = 0;
      for (int j = 0; j < listOfPair.size(); ++j) {
        if (pageList.at(pageReport)->getBand(DataGroupHeader) != 0 and listOfPair.at(j).pageReport == pageReport and
            listOfPair.at(j).paramName == pageList.at(pageReport)->getBand(DataGroupHeader)->groupingField) {
          bool founded = false;
          for (int i = 0; i < listOfGroup.size(); ++i) {
            if (listOfGroup.at(i) == listOfPair.at(j).paramValue) founded = true;
          }

          listIdxOfGroup.clear();
          for (int k = 0; k < listOfPair.size(); ++k) {
            if (listOfPair.at(k).paramName == pageList.at(pageReport)->getBand(DataGroupHeader)->groupingField and
                listOfPair.at(k).pageReport == pageReport and
          ...
          if (not founded) { // Start new group
            grpNo += 1;
            ...
            if (pageList.at(pageReport)->getBand(DataGroupHeader)->startNewPage == 1 and
                grpNo != 1) // Start new page for each Data group
              newPage(printer, y, draw);

            m_recNo = listOfPair.at(j).lnNo;

            // Start new numeration for group
            if (pageList.at(pageReport)->getBand(DataGroupHeader)->startNewNumeration != 0) mg_recNo = 0;
            ...
            bandObj = pageList.at(pageReport)->getBand(MasterHeader);
            if (bandObj != 0 and bandObj->showInGroup == 1) processMHeader(y, draw);
            processPFooter(draw);
            processMasterData(printer, y, draw, pageReport);

            bandObj = pageList.at(pageReport)->getBand(MasterFooter);
            if (bandObj != 0 and bandObj->showInGroup == 1) processMFooter(printer, y, draw);
            //---
            m_recNo = listOfPair.at(j).lnNo;
      ...
      if (pageList.at(pageReport)->getBand(MasterFooter) != 0 and
          pageList.at(pageReport)->getBand(MasterFooter)->showInGroup == 0)
        processMFooter(printer, y, draw);
    }
  }
}
...
void QtRPT::processMasterData(QPrinter *printer, int &y, bool draw, int pageReport) {
  if (not recordCount.isEmpty()) {
    if (pageReport < recordCount.size() and recordCount.at(pageReport) > 0) {
      if (pageList.at(m_pageReport)->getBand(MasterData) != 0) {
        for (int i = 0; i < recordCount.at(pageReport); i++) {
          ...
          if (listIdxOfGroup.size() > 0) { // If report with groups, we checking that current line in the current group
            for (int k = 0; k < listIdxOfGroup.size(); k++) {
              if (listIdxOfGroup.at(k) == i) founded = true;
            }
          } else {
            founded = true;
          }
...
void QtRPT::processMFooter(QPrinter *printer, int &y, bool draw) {
  ...
  if (y > ph - mb - mt - pageList.at(m_pageReport)->getBand(MasterFooter)->height) newPage(printer, y, draw);
...
void QtRPT::processRSummary(QPrinter *printer, int &y, bool draw) {
  ...
  if (y + pageList.at(m_pageReport)->getBand(ReportSummary)->height >
      ph - mb - mt - pageList.at(m_pageReport)->getBand(ReportSummary)->height)
    newPage(printer, y, draw);
...
void QtRPT::openDataSource(int pageReport) {
  ...
  while (not n.isNull()) {
    ...
    if ((not e.isNull()) and (e.tagName() == "DataSource")) {
      dsElement = e;
    }
    n = n.nextSibling();
  }

  if (not dsElement.isNull() and dsElement.attribute("type") == "SQL") {
    ...
    if (not m_sqlQuery.isEmpty()) sqlQuery = m_sqlQuery;
    if (not rptSql->openQuery(sqlQuery, dbCoding, charsetCoding)) {
      recordCount << 0;
      return;
    }
  ...
  if (not dsElement.isNull() and dsElement.attribute("type") == "XML") {
  }
}

/*!
 \fn QtRPT::setSqlQuery(QString sqlString)
  QtRPT::setSqlQuery sets SQL query a\sqlString.
  If in XML was defiened SQL query, it will be replaced.
 */
void QtRPT::setSqlQuery(QString sqlString) { m_sqlQuery = sqlString; }
