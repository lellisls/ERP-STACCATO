...
QObjectPropertyModel::QObjectPropertyModel(QObject *parent /*=0*/)
    : QAbstractItemModel(parent), m_rootNode(0), m_object(0), m_dataChanging(false), m_subclassesAsLevel(true),
      m_validator(0) {}
...
void QObjectPropertyModel::initModel() {
  ...
  if (m_object) {
    ...
    for (QObject *item : m_objects)
      connect(item, &QObject::destroyed, this, &QObjectPropertyModel::slotObjectDestroyed);
...
void QObjectPropertyModel::slotPropertyChanged(const QString &propertyName, const QVariant &oldValue,
...
void QObjectPropertyModel::updateProperty(const QString &propertyName) {
  if (!m_dataChanging and m_rootNode) {
...
Qt::ItemFlags QObjectPropertyModel::flags(const QModelIndex &index) const {
  if ((index.column() == 1) and (!nodeFromIndex(index)->isValueReadonly()))
...
CreatePropItem QObjectPropertyModel::propertyItemCreator(QMetaProperty prop) {
  ...
  creator =
      ObjectPropFactory::instance().objectCreator(APropIdent(prop.name(), prop.enclosingMetaObject()->className()));
  if (!creator) {
    if (prop.isFlagType()) {
    ...
    if (!creator) {
      qDebug() << "Editor for propperty name = \"" << prop.name() << "\" & property type =\"" << prop.typeName()
               << "\" not found!";
    }
  }
  return creator;
...
ObjectPropItem *QObjectPropertyModel::createPropertyItem(QMetaProperty prop, QObject *object,
                                                         ObjectPropItem::ObjectsList *objects, ObjectPropItem *parent) {
  ObjectPropItem *propertyItem = 0;
  CreatePropItem creator = propertyItemCreator(prop);

  if (creator) {
    propertyItem = creator(object, objects, QString(prop.name()),
                           QString(tr(prop.name())), //сделать перевод значений на другие языки
                           object->property(prop.name()), parent, !(prop.isWritable() and prop.isDesignable()));
  } else {
    propertyItem =
        new ObjectPropItem(0, 0, QString(prop.name()), QString(prop.name()), object->property(prop.name()), parent);
  }
  return propertyItem;
}
...
bool QObjectPropertyModel::setData(const QModelIndex &index, const QVariant &value, int role) {
  if (index.isValid() and role == Qt::EditRole) {
    ...
    if (propItem->propertyValue() != value) {
      QString msg;
      if (validator() and !validator()->validate(propItem->propertyName(), value.toString(), m_object, msg)) {
