...
ReportEnginePrivate::ReportEnginePrivate(QObject *parent)
    : QObject(parent), m_fileName(""), m_settings(0), m_ownedSettings(false),
      ...
      m_reportName(""), m_activePreview(0), m_previewWindowIcon(":/report/images/logo32"),
      m_previewWindowTitle(tr("Preview")) {
  m_datasources = new DataSourceManager(this);
  m_datasources->setObjectName("datasources");
  connect(m_datasources, &DataSourceManager::loadCollectionFinished, this,
...
void ReportEnginePrivate::collectionLoadFinished(const QString &) {
  foreach (PageDesignIntf *page, m_pages) {
    page->setReportEditor(this);
    page->setSceneRect(-Const::SCENE_MARGIN, -Const::SCENE_MARGIN, page->pageItem()->width() + Const::SCENE_MARGIN * 2,
...
void ReportEnginePrivate::printReport(ReportPages pages, QPrinter &printer, const PrintRange &printRange) {
  ...
  foreach (PageItemDesignIntf::Ptr page, pages) {
    ...
    if ((printRange.rangeType() == QPrintDialog::AllPages) or
        ((printRange.rangeType() == QPrintDialog::PageRange) and (currenPage >= printRange.fromPage()) and
      ...
      if (renderPage.pageItem()->oldPrintMode()) {
        printer.setPageMargins(renderPage.pageItem()->leftMargin(), renderPage.pageItem()->topMargin(),
                               renderPage.pageItem()->rightMargin(), renderPage.pageItem()->bottomMargin(),
                               QPrinter::Millimeter);
        printer.setOrientation((QPrinter::Orientation)renderPage.pageItem()->pageOrientation());
        QSizeF pageSize =
            (renderPage.pageItem()->pageOrientation() == PageItemDesignIntf::Landscape)
                ? QSizeF(renderPage.pageItem()->sizeMM().height(), renderPage.pageItem()->sizeMM().width())
                : renderPage.pageItem()->sizeMM();
        printer.setPaperSize(pageSize, QPrinter::Millimeter);
      } else {
...
void ReportEnginePrivate::previewReport() {
  ...
  try {
    ...
    if (pages.count() > 0) {
      ...
      m_activePreview = w;
      connect(w, &QObject::destroyed, this, &ReportEnginePrivate::slotPreviewWindowDestroed);
...
ReportPages ReportEnginePrivate::renderToPages() {
  ...
  dataManager()->connectAllDatabases();
  dataManager()->setDesignTime(false);
  connect(m_reportRender.data(), &ReportRender::pageRendered, this, &ReportEnginePrivate::renderPageFinished);
  if (m_pages.count()) {
    emit renderStarted();
...
ReportEngine::ReportEngine(QObject *parent) : QObject(parent), d_ptr(new ReportEnginePrivate()) {
  Q_D(ReportEngine);
  d->q_ptr = this;
  connect(d, &ReportEnginePrivate::renderStarted, this, &ReportEngine::renderStarted);
  connect(d, &ReportEnginePrivate::renderPageFinished, this, &ReportEngine::renderPageFinished);
  connect(d, &ReportEnginePrivate::renderFinished, this, &ReportEngine::renderFinished);
  connect(d, &ReportEnginePrivate::onSave, this, &ReportEngine::onSave);
  connect(d, &ReportEnginePrivate::onLoad, this, &ReportEngine::onLoad);
  connect(d, &ReportEnginePrivate::saveFinished, this, &ReportEngine::saveFinished);
}
