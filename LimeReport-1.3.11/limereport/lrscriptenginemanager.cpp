...
ScriptEngineModel::ScriptEngineModel(ScriptEngineManager *scriptManager) : m_rootNode(new ScriptEngineNode()) {
...
QScriptValue numberFormat(QScriptContext *pcontext, QScriptEngine *pengine) {
  ...
  QScriptValue res = (locale.isEmpty())
                         ? pengine->newVariant(QString::number(value.toDouble(), format, precision))
...
QScriptValue ScriptEngineManager::addFunction(const QString &name, QScriptEngine::FunctionSignature function,
...
QScriptValue ScriptEngineManager::addFunction(const QString &name, const QString &script, const QString &category,
...
ScriptEngineManager::ScriptEngineManager() : m_model(0), m_dataManager(0) {
  ...
  addFunction("dateTimeFormat", dateTimeFormat, "DATE&TIME",
              "dateTimeFormat(\"" + tr("Value") + "\",\"" + tr("Format") + "\")");
  ...
  while (curPos < m_context.length()) {
    ...
    case None:
      if (m_context[curPos] == '$') {
        ...
        if (isStartFieldLexem(curPos) or isStartVariableLexem(curPos)) skipField(curPos);
      }
    default:
      break;
    }
