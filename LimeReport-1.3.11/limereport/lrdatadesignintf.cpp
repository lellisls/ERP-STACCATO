...
void QueryHolder::fillParams(QSqlQuery *query) {
  foreach (QString param, m_aliasesToParam.keys()) {
    ...
    if (value.isValid() or m_mode == IDataSource::DESIGN_MODE) query->bindValue(':' + param, value);
  }
}

void QueryHolder::extractParams() { m_preparedSQL = replaceVariables(m_queryText); }
...
void QueryHolder::setQueryText(QString queryText) { m_queryText = queryText; }

IDataSource *QueryHolder::dataSource(IDataSource::DatasourceMode mode) {
  if ((m_mode != mode and m_mode == IDataSource::DESIGN_MODE) or m_dataSource == 0) {
    m_mode = mode;
    runQuery(mode);
  }
  if (m_dataSource)
...
ModelToDataSource::~ModelToDataSource() {
  if ((m_owned) and m_model != 0) delete m_model;
}
...
bool ModelToDataSource::eof() {
  ...
  return (m_curRow == m_model->rowCount()) or (m_model->rowCount() == 0);
}

bool ModelToDataSource::bof() {
  if (isInvalid()) return true;
  return (m_curRow == -1) or (m_model->rowCount() == 0);
}
...
ConnectionDesc::ConnectionDesc(QSqlDatabase db, QObject *parent)
    : QObject(parent), m_connectionName(db.connectionName()), m_connectionHost(db.hostName()),
      ...
      m_password(db.password()), m_autoconnect(false) {}

ConnectionDesc::ConnectionDesc(QObject *parent)
    : QObject(parent), m_connectionName(""), m_connectionHost(""), m_connectionDriver(""), m_databaseName(""),
      m_user(""), m_password(""), m_autoconnect(false) {}
    ...
    while ((pos = rx.indexIn(query)) != -1) {
      ...
      if (!m_aliasesToParam.contains(field)) {
        ...
        if (field.contains("."))
          ...
          m_aliasesToParam.insert(field + "_alias" + QString::number(curentAliasIndex),
                                  m_masterDatasource + "." + field);
  ...
  if (!m_datasource) {
    ...
    if (dataManager()) {
      ...
      if (master and child) {
        ...
        if (!child)
          m_lastError += ((m_lastError.isEmpty()) ? QObject::tr("Child") : QObject::tr(" and child ")) +
...
void ProxyHolder::invalidate(IDataSource::DatasourceMode mode) {
  Q_UNUSED(mode)
  if (m_model and m_model->isInvalid()) {
    m_invalid = true;
...
bool MasterDetailProxyModel::isInvalid() const {
  if (m_masterName.isEmpty() or m_childName.isEmpty()) return true;
  IDataSource *masterData = dataManager()->dataSource(m_masterName);
  IDataSource *childData = dataManager()->dataSource(m_childName);
  if (!masterData or !childData) return true;
  return masterData->isInvalid() or childData->isInvalid();
}
...
bool CallbackDatasource::next() {
  if (!m_eof) {
    ...
    if (m_rowCount != -1) {
      if (m_rowCount > 0 and m_currentRow < m_rowCount) {
        m_eof = false;
      } else {
        m_eof = true;
      }
...
int CallbackDatasource::columnIndexByName(QString name) {
  for (int i = 0; i < m_headers.size(); ++i) {
    if (m_headers[i].compare(name) == 0) return i;
  }
  //    if (m_headers.size()==0){
  //        QVariant data;
  //        bool hasNextRow;
