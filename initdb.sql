...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_estoque` (`Fornecedor` INT, `Produto` INT, `Quant` INT, `un` INT, `un2` INT, `Caixas M2` INT, `Caixas PÇ` INT, `Cód Com` INT, `Status` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produto_pendente` (`fornecedor` INT, `idVenda` INT, `idProduto` INT, `produto` INT, `colecao` INT, `formComercial` INT, `quant` INT, `un` INT, `Un2` INT, `Caixas` INT, `kgcx` INT, `codComercial` INT, `codBarras` INT, `custo` INT, `idCompra` INT, `status` INT, `Mês` INT, `estoque` INT);
...
VIEW `mydb`.`view_produto_pendente` AS
    SELECT 
        ...
        (SELECT 
                SUM(`mydb`.`estoque`.`quant`)
            FROM
                `mydb`.`estoque`
            WHERE
                (`mydb`.`estoque`.`codComercial` = `v`.`codComercial`)) AS `estoque`
    FROM
        (((`mydb`.`venda_has_produto` `v`
        LEFT JOIN `mydb`.`estoque` `e` ON ((`v`.`idProduto` = `e`.`idProduto`)))
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_venda_produto` AS select `v`.`data` AS `data`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`idVenda` AS `idVenda`,`vp`.`produto` AS `produto`,`vp`.`caixas` AS `caixas`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`codComercial` AS `codComercial`,`vp`.`formComercial` AS `formComercial`,`vp`.`status` AS `status` from (`mydb`.`venda` `v` left join `mydb`.`venda_has_produto` `vp` on((`v`.`idVenda` = `vp`.`idVenda`)));
