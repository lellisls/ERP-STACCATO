...
CREATE TABLE IF NOT EXISTS `mydb`.`estoque` (
  ...
  `local` VARCHAR(100) NOT NULL DEFAULT 'TEMP',
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fornecedor_compra` (`fornecedor` INT, `COUNT(fornecedor)` INT);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_compras` AS select `mydb`.`pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,`mydb`.`pedido_fornecedor_has_produto`.`idCompra` AS `idCompra`,count(`mydb`.`pedido_fornecedor_has_produto`.`idProduto`) AS `COUNT(idProduto)`,sum(`mydb`.`pedido_fornecedor_has_produto`.`preco`) AS `SUM(preco)`,`mydb`.`pedido_fornecedor_has_produto`.`status` AS `status` from `mydb`.`pedido_fornecedor_has_produto` where (`mydb`.`pedido_fornecedor_has_produto`.`status` = 'EM COMPRA') group by `mydb`.`pedido_fornecedor_has_produto`.`idCompra`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fornecedor_compra` AS select `mydb`.`pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,count(`mydb`.`pedido_fornecedor_has_produto`.`fornecedor`) AS `COUNT(fornecedor)` from `mydb`.`pedido_fornecedor_has_produto` where (`mydb`.`pedido_fornecedor_has_produto`.`status` <> 'FINALIZADO') group by `mydb`.`pedido_fornecedor_has_produto`.`fornecedor`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fornecedor_logistica` AS select `mydb`.`pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,count(`mydb`.`pedido_fornecedor_has_produto`.`fornecedor`) AS `COUNT(fornecedor)` from `mydb`.`pedido_fornecedor_has_produto` where (`mydb`.`pedido_fornecedor_has_produto`.`status` <> 'FINALIZADO') group by `mydb`.`pedido_fornecedor_has_produto`.`fornecedor`;
...
VIEW `mydb`.`view_orcamento` AS
    ...
    FROM
        ...
        LEFT JOIN `mydb`.`usuario` ON `mydb`.`orcamento`.`idUsuario` = `mydb`.`usuario`.`idUsuario`);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_pendente` AS select `v`.`fornecedor` AS `fornecedor`,`v`.`produto` AS `produto`,`v`.`formComercial` AS `formComercial`,sum(`v`.`quant`) AS `SUM(v.quant)`,`v`.`un` AS `un`,`v`.`codComercial` AS `codComercial`,`v`.`idCompra` AS `idCompra`,`v`.`status` AS `status`,(select sum(`mydb`.`estoque`.`quant`) from `mydb`.`estoque` where (`mydb`.`estoque`.`codComercial` = `v`.`codComercial`)) AS `quant` from (`mydb`.`venda_has_produto` `v` left join `mydb`.`estoque` `e` on((`v`.`idProduto` = `e`.`idProduto`))) group by `v`.`status`,`v`.`codComercial`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_produtos_pendentes` AS select `v`.`fornecedor` AS `Fornecedor`,`v`.`produto` AS `Descrição`,`v`.`formComercial` AS `Form`,sum(`v`.`quant`) AS `Quant`,`v`.`un` AS `Un`,`v`.`codComercial` AS `Cód Com`,`v`.`idCompra` AS `idCompra`,`v`.`status` AS `Status`,(select sum(`mydb`.`estoque`.`quant`) from `mydb`.`estoque` where (`mydb`.`estoque`.`codComercial` = `v`.`codComercial`)) AS `Estoque` from (`mydb`.`venda_has_produto` `v` left join `mydb`.`estoque` `e` on((`v`.`idProduto` = `e`.`idProduto`))) group by `v`.`status`,`v`.`codComercial`;
...
VIEW `mydb`.`view_venda` AS
    SELECT 
        ...
        `v`.`total` AS `Total R$`,
        `v`.`status` AS `Status`,
        (TO_DAYS(`v`.`prazoEntrega`) - TO_DAYS(NOW())) AS `Dias restantes`
    FROM
        (((((`mydb`.`venda` `v`
