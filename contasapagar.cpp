...
void ContasAPagar::on_pushButtonSalvar_clicked() {
  if (ui->checkBoxPago->isChecked()) {
    QSqlQuery qry;
    if (!qry.exec("UPDATE contaapagar SET pago = 'SIM' WHERE idVenda = '" + idVenda + "'")) {
      qDebug() << "Erro ao marcar conta como paga: " << qry.lastError();
    }
  } else {
    QSqlQuery qry;
    if (!qry.exec("UPDATE contaapagar SET pago = 'NÃO' WHERE idVenda = '" + idVenda + "'")) {
