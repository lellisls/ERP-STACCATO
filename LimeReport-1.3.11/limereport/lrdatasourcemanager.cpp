...
DataSourceModel::DataSourceModel(DataSourceManager *dataManager) : m_dataManager(0), m_rootNode(new DataNode()) {
...
QModelIndex DataSourceModel::parent(const QModelIndex &child) const {
  ...
  if ((parentNode == m_rootNode) or (!parentNode)) return QModelIndex();
...
DataSourceManager::DataSourceManager(QObject *parent)
  ...
  m_groupFunctionFactory.registerFunctionCreator(QLatin1String("COUNT"),
                                                 new ConstructorGroupFunctionCreator<CountGroupFunction>);
  m_groupFunctionFactory.registerFunctionCreator(QLatin1String("SUM"),
                                                 new ConstructorGroupFunctionCreator<SumGroupFunction>);
  m_groupFunctionFactory.registerFunctionCreator(QLatin1String("AVG"),
                                                 new ConstructorGroupFunctionCreator<AvgGroupFunction>);
  m_groupFunctionFactory.registerFunctionCreator(QLatin1String("MIN"),
                                                 new ConstructorGroupFunctionCreator<MinGroupFunction>);
  m_groupFunctionFactory.registerFunctionCreator(QLatin1String("MAX"),
...
bool DataSourceManager::addModel(const QString &name, QAbstractItemModel *model, bool owned) {
  ...
  try {
    putHolder(name, mh);
    connect(mh, &ModelHolder::modelStateChanged, this, &DataSourceManager::datasourcesChanged);
  } catch (ReportError &e) {
    putError(e.what());
    setLastError(e.what());
    return false;
...
QSharedPointer<QAbstractItemModel> DataSourceManager::previewSQL(const QString &connectionName, const QString &sqlText,
  ...
  if (db.isValid() and db.isOpen()) {
    ...
    foreach (QString param, aliasesToParam.keys()) {
      ...
      if (value.isValid() or m_designTime) query.bindValue(':' + param, value);
    }
  ...
  if (!db.isOpen()) m_lastError = tr("Connection \"%1\" is not open").arg(connectionName);
  return QSharedPointer<QAbstractItemModel>(0);
}
...
void DataSourceManager::addSubQuery(const QString &name, const QString &sqlText, const QString &connectionName,
...
void DataSourceManager::addConnectionDesc(ConnectionDesc *connection) {
  if (!isConnection(connection->name())) {
    connect(connection, &ConnectionDesc::nameChanged, this, &DataSourceManager::slotConnectionRenamed);
...
void DataSourceManager::disconnectConnection(const QString &connectionName) {
  foreach (QString datasourceName, dataSourceNames()) {
    if (isQuery(datasourceName) or isSubQuery(datasourceName)) {
      QueryHolder *qh = dynamic_cast<QueryHolder *>(dataSourceHolder(datasourceName));
      if (qh and qh->connectionName().compare(connectionName, Qt::CaseInsensitive) == 0) {
        qh->invalidate(designTime() ? IDataSource::DESIGN_MODE : IDataSource::RENDER_MODE);
...
void DataSourceManager::collectionLoadFinished(const QString &collectionName) {
  ...
  if (collectionName.compare("subqueries", Qt::CaseInsensitive) == 0) {
    foreach (SubQueryDesc *query, m_subqueries) {
      putHolder(query->queryName(),
                new SubQueryHolder(query->queryText(), query->connectionName(), query->master(), this));
    }
  }

  if (collectionName.compare("subproxies", Qt::CaseInsensitive) == 0) {
    foreach (ProxyDesc *proxy, m_proxies) { putHolder(proxy->name(), new ProxyHolder(proxy, this)); }
  }
...
void DataSourceManager::addVariable(const QString &name, const QVariant &value, VarDesc::VarType type,
                                    RenderPass pass) {
...
void DataSourceManager::deleteVariable(const QString &name) {
  if (m_varHolder.containsVariable(name) and m_varHolder.variableType(name) == VarDesc::User) {
...
void DataSourceManager::setLastError(const QString &value) {
  m_lastError = value;
  if (!value.isEmpty() and !m_errorsList.contains(value)) {
...
void DataSourceManager::invalidateLinkedDatasources(QString datasourceName) {
  foreach (QString name, dataSourceNames()) {
    ...
    if (isProxy(name)) {
      ...
      if ((proxy->master() == datasourceName) or (proxy->child() == datasourceName))
        dataSourceHolder(name)->invalidate(designTime() ? IDataSource::DESIGN_MODE : IDataSource::RENDER_MODE);
    }
  }
}
...
void DataSourceManager::clear(ClearMethod method) {
  ...
  for (dit = m_datasources.begin(); dit != m_datasources.end();) {
    bool owned = (*dit)->isOwned() and (*dit)->isRemovable();
    switch (method) {
    case All:
...
GroupFunction *DataSourceManager::addGroupFunction(const QString &name, const QString &expression, const QString &band,
...
GroupFunction *DataSourceManager::groupFunction(const QString &name, const QString &expression, const QString &band) {
  foreach (GroupFunction *gf, m_groupFunctions.values(band)) {
    if ((gf->name().compare(name, Qt::CaseInsensitive) == 0) and
...
void DataSourceManager::clearUserVariables() { m_varHolder.clearUserVariables(); }
...
QVariant DataSourceManager::variable(const QString &variableName) { return m_varHolder.variable(variableName); }
...
QStringList DataSourceManager::variableNames() { return m_varHolder.variableNames(); }

VarDesc::VarType DataSourceManager::variableType(const QString &name) { return m_varHolder.variableType(name); }
