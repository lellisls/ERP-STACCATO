...
bool ReportRender::isNeedToRearrangeColumnsItems() {
  ...
  for (int i = 0; i < m_maxHeightByColumn.size(); ++i) {
    ...
    if (maxHeightColumn > 0 and columnItemsCount(maxHeightColumn) < avg and
...
ReportRender::ReportRender(QObject *parent)
    : QObject(parent), m_renderPageItem(0), m_pageCount(0), m_currentColumn(0), m_lastDataBand(0),
      m_lastRenderedFooter(0) {
  initColumns();
}

void ReportRender::setDatasources(DataSourceManager *value) { m_datasources = value; }
...
void ReportRender::renderPage(PageDesignIntf *patternPage) {
  ...
  for (int i = 0; i < m_patternPageItem->dataBandCount() and !m_renderCanceled; i++) {
  ...
  if (reportFooter) renderBand(reportFooter, StartNewPageAsNeeded);
  if (lastRenderedBand and lastRenderedBand->keepFooterTogether()) closeFooterGroup(lastRenderedBand);

  savePage();
  if (!m_renderCanceled) secondRenderPass();
}

int ReportRender::pageCount() { return m_renderedPages.count(); }

PageItemDesignIntf::Ptr ReportRender::pageAt(int index) {
  if ((index > m_renderedPages.count() - 1) or (index < 0))
    throw ReportError("page index out of range");
  else
...
void ReportRender::extractGroupsFunction(BandDesignIntf *band) {
  for (BaseDesignIntf *item : band->childBaseItems()) {
    ContentItemDesignIntf *contentItem = dynamic_cast<ContentItemDesignIntf *>(item);
    if (contentItem and (contentItem->content().contains(QRegExp("\\$S\\s*\\{.*\\}")))) {
      for (QString functionName : m_datasources->groupFunctionNames()) {
        ...
        if (rx.indexIn(contentItem->content()) >= 0) {
          ...
          if (dataBand) {
            GroupFunction *gf = datasources()->addGroupFunction(functionName, rx.cap(Const::VALUE_INDEX),
                                                                band->objectName(), dataBand->objectName());
            if (gf) {
              connect(dataBand, &BandDesignIntf::bandRendered, gf, &GroupFunction::slotBandRendered);
            }
          } else {
            GroupFunction *gf = datasources()->addGroupFunction(functionName, rx.cap(Const::VALUE_INDEX),
                                                                band->objectName(), rx.cap(Const::DATASOURCE_INDEX));
    ...
    if (contentItem) {
      ...
      foreach (QString functionName, m_datasources->groupFunctionNames()) {
        ...
        if (rx.indexIn(content) >= 0) {
          content.replace(
              rx,
              QString("%1(%2,%3)").arg(functionName).arg('"' + rx.cap(4) + '"').arg('"' + band->objectName() + '"'));
...
void ReportRender::renderBand(BandDesignIntf *patternBand, ReportRender::DataRenderMode mode, bool isLast) {
  ...
  if (patternBand) {
    ...
    if (isLast and bandClone->keepFooterTogether() and bandClone->sliceLastRow()) {
      if (m_maxHeightByColumn[m_currentColumn] < (bandClone->height() + m_reportFooterHeight))
        m_maxHeightByColumn[m_currentColumn] -= ((m_maxHeightByColumn[m_currentColumn] - bandClone->height()) +
                                                 (bandClone->height() * calcSlicePercent(bandClone->height())));
    }

    if (!bandClone->isEmpty() or patternBand->printIfEmpty()) {
        ...
        if (bandClone->canBeSplitted(m_maxHeightByColumn[m_currentColumn])) {
          ...
          if (bandClone->maxScalePercent() >= percent) {
            ...
            if (mode == StartNewPageAsNeeded) {
              if (bandClone->columnsCount() > 1 and
                  (bandClone->columnsFillDirection() == BandDesignIntf::Vertical or
...
void ReportRender::renderDataBand(BandDesignIntf *dataBand) {
  ...
  m_lastRenderedFooter = 0;
  if (dataBand and !dataBand->datasourceName().isEmpty())
    bandDatasource = datasources()->dataSource(dataBand->datasourceName());

  if (bandDatasource and !bandDatasource->eof() and !m_renderCanceled) {
    ...
    if (dataBand->bandHeader() and dataBand->bandHeader()->reprintOnEachPage())
    ...
    while (!bandDatasource->eof() and !m_renderCanceled) {

      if ((firstTime and dataBand->startFromNewPage()) or (!firstTime and dataBand->startNewPage())) {
        savePage();
        startNewPage();
      }
      ...
      if (dataBand->keepFooterTogether() and !bandDatasource->hasNext()) openFooterGroup(dataBand);
...
void ReportRender::renderPageFooter(PageItemDesignIntf *patternPage) {
  ...
  if (band) {
    BandDesignIntf *bandClone =
        dynamic_cast<BandDesignIntf *>(band->cloneItem(PreviewMode, m_renderPageItem, m_renderPageItem));
    replaceGroupsFunction(bandClone);
    bandClone->updateItemSize(m_datasources);
    bandClone->setItemPos(m_patternPageItem->pageRect().x(),
                          m_patternPageItem->pageRect().bottom() - bandClone->height());
...
void ReportRender::renderPageItems(PageItemDesignIntf *patternPage) {
  foreach (BaseDesignIntf *item, patternPage->childBaseItems()) {
    ...
    if (id and id->itemLocation() == ItemDesignIntf::Page) {
...
void ReportRender::renderChildFooter(BandDesignIntf *parent, BandPrintMode printMode) {
  foreach (BandDesignIntf *band, parent->childrenByType(BandDesignIntf::SubDetailFooter)) {
    ...
    if ((band != m_lastRenderedFooter) and (printAlways == (printMode == PrintAlwaysPrintable)))
...
void ReportRender::renderGroupHeader(BandDesignIntf *parentBand, IDataSource *dataSource, bool firstTime) {
  foreach (BandDesignIntf *band, parentBand->childrenByType(BandDesignIntf::GroupHeader)) {
    ...
    if (!gb->isStarted()) {
      ...
      openDataGroup(band);
      if (!firstTime and gb->startNewPage()) {
...
void ReportRender::closeGroup(BandDesignIntf *band) {
  ...
  auto it = m_childBands.find(band);

  while (it != m_childBands.end() and it.key() == band) {
...
bool ReportRender::registerBand(BandDesignIntf *band, bool registerInChildren) {
  if (band->columnsCount() == 1 and m_maxHeightByColumn.size() > 1) {
  ...
  if ((band->columnsCount() > 1) and !band->isHeader()) {
  ...
  if (band->height() <= m_maxHeightByColumn[m_currentColumn]) {
    ...
    if (band->isHeader() and band->columnsCount() > 1) {
    ...
    foreach (QList<BandDesignIntf *> *list, m_childBands.values()) {
      if (registerInChildren and band->bandType() != BandDesignIntf::PageHeader and
          band->bandType() != BandDesignIntf::PageFooter and band->bandType() != BandDesignIntf::ReportHeader and
          band->bandType() != BandDesignIntf::ReportFooter and !list->contains(band) and !band->reprintOnEachPage())
        list->append(band);
    }
...
BandDesignIntf *ReportRender::sliceBand(BandDesignIntf *band, BandDesignIntf *patternBand, bool isLast) {
  while (band->height() > m_maxHeightByColumn[m_currentColumn]) {
    ...
    if (!band->isEmpty()) {
      ...
      if (isLast and data and data->keepFooterTogether() and band->height() < m_maxHeightByColumn[m_currentColumn] and
          band->sliceLastRow()) {
        if (band->height() > (m_maxHeightByColumn[m_currentColumn] - m_reportFooterHeight)) {
          m_maxHeightByColumn[m_currentColumn] -= ((m_maxHeightByColumn[m_currentColumn] - band->height()) +
...
void ReportRender::secondRenderPass() {
  for (int i = 0; i < m_renderedPages.count(); ++i) {
    ...
    foreach (BandDesignIntf *band, page->childBands()) { band->updateItemSize(m_datasources, SecondPass); }
  }
}
...
BandDesignIntf *ReportRender::saveUppperPartReturnBottom(BandDesignIntf *band, int height,
  ...
  if (band->columnsCount() > 1 and (band->columnsFillDirection() == BandDesignIntf::Vertical or
  ...
  //    if (!bottomBandPart->isEmpty() and patternBand->keepFooterTogether())
...
void ReportRender::startNewPage() {
  ...
  foreach (BandDesignIntf *band, m_reprintableBands) { renderBand(band); }
...
int ReportRender::findLastPageNumber(int currentPage) {
  foreach (PagesRange range, m_ranges) {
    if (range.firstPage <= (currentPage + 1) and range.lastPage >= (currentPage + 1))
...
void ReportRender::checkFooterGroup(BandDesignIntf *groupBand) {
  if (m_childBands.contains(groupBand)) {
    ...
    foreach (BandDesignIntf *band, *holder) {
      ...
      if (m_renderedDataBandCount <= 1 or percent > 20) {
...
BandDesignIntf *ReportRender::findEnclosingGroup() {
  ...
  if (!m_childBands.isEmpty()) {
    ...
    foreach (BandDesignIntf *gb, m_childBands.keys()) {
      if (m_childBands.value(gb)->tryToKeepTogether() and ((gb->bandIndex() < groupIndex) or (groupIndex == -1))) {
        result = gb;
...
void ReportRender::savePage() {
  ...
  if (m_ranges.last().lastPage == 0 and m_ranges.count() > 1) {
    m_datasources->setReportVariable("#PAGE", 1);
  } else {
