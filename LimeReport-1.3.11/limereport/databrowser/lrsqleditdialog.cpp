...
void SQLEditDialog::check() {
  ...
  if (ui->textEditSQL->toPlainText().isEmpty() and (!ui->rbProxy)) throw LimeReport::ReportError(tr("SQL is empty !"));
...
void SQLEditDialog::setDefaultConnection(QString defaultConnection) { m_defaultConnection = defaultConnection; }
...
void SQLEditDialog::on_cbSubdetail_clicked(bool checked) {
  ...
  if ((checked) and (ui->rbProxy->isChecked())) initProxyMode();
  if ((checked) and (ui->rbSubQuery->isChecked())) initSubQueryMode();
...
void SQLEditDialog::slotPreviewData() {
  ...
  m_previewModel =
      m_datasources->previewSQL(ui->cbbConnection->currentText(), ui->textEditSQL->toPlainText(), ui->leMaster->text());
  if (m_previewModel) {
