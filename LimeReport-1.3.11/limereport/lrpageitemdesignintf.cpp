...
PageItemDesignIntf::PageItemDesignIntf(QObject *owner, QGraphicsItem *parent)
    : BaseDesignIntf("PageItem", owner, parent), m_topMargin(0), m_bottomMargin(0), m_leftMargin(0), m_rightMargin(0),
      m_pageOrientaion(Portrait), m_pageSize(A4), m_sizeChainging(false), m_fullPage(false), m_oldPrintMode(false) {
  setFixedPos(true);
...
void PageItemDesignIntf::geometryChangedEvent(QRectF newRect, QRectF) {
  ...
  if (!m_sizeChainging and !isLoading()) m_pageSize = Custom;
...
int PageItemDesignIntf::calcBandIndex(BandDesignIntf::BandsType bandType, BandDesignIntf *parentBand,
  ...
  foreach (BandDesignIntf *band, m_bands) {
    if ((band->bandType() == BandDesignIntf::GroupHeader) and (band->bandType() > bandType)) break;
  ...
  if (parentBand) {
    ...
    while (dataBand->parentBand() and dataBand->bandType() == BandDesignIntf::GroupHeader) {
...
void PageItemDesignIntf::registerBand(BandDesignIntf *band) {
  if (!isBandRegistred(band)) {
    ...
    connect(band, &QObject::destroyed, this, &PageItemDesignIntf::bandDeleted);
    connect(band, &BaseDesignIntf::geometryChanged, this, &PageItemDesignIntf::bandGeometryChanged);
  }
}
...
void PageItemDesignIntf::relocateBands() {
  ...
  if (m_bands.count() > 1) {
    ...
    for (int i = 0; i < (m_bands.count() - 1); i++) {
      if ((m_bands[i + 1]->bandType() != BandDesignIntf::PageFooter) or (itemMode() & DesignMode)) {
        if (m_bands[i + 1]->columnsCount() > 1 and m_bands[i]->columnsCount() != m_bands[i + 1]->columnsCount()) {
...
void PageItemDesignIntf::setPageOrientation(PageItemDesignIntf::Orientation value) {
  if (!m_sizeChainging and m_pageOrientaion != value) {
    m_sizeChainging = true;
...
void PageItemDesignIntf::updateMarginRect() {
  m_pageRect = rect();
  m_pageRect.adjust(m_leftMargin * mmFactor(), m_topMargin * mmFactor(), -m_rightMargin * mmFactor(),
                    -m_bottomMargin * mmFactor());
  foreach (BandDesignIntf *band, m_bands) {
    band->setWidth(pageRect().width() / band->columnsCount());
    relocateBands();
  }
...
void PageItemDesignIntf::paintGrid(QPainter *ppainter) {
  ...
  for (int i = 0; i <= (pageRect().height() - 50) / 100; i++) {
    ppainter->drawLine(pageRect().x(), (i * 100) + pageRect().y() + 50, pageRect().right(),
                       i * 100 + pageRect().y() + 50);
  };
  for (int i = 0; i <= ((pageRect().width() - 50) / 100); i++) {
    ppainter->drawLine(i * 100 + pageRect().x() + 50, pageRect().y(), i * 100 + pageRect().x() + 50,
                       pageRect().bottom());
  };
