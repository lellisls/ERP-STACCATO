...
ReportDesignWindow::ReportDesignWindow(ReportEnginePrivate *report, QWidget *parent, QSettings *settings)
    : QMainWindow(parent), m_textAttibutesIsChanging(false), m_settings(settings), m_ownedSettings(false),
      m_progressDialog(0), m_showProgressDialog(true) {
...
ReportDesignWindow::~ReportDesignWindow() {
  m_instance = 0;
  delete m_validator;
  if (m_ownedSettings and m_settings) delete m_settings;
}
...
void ReportDesignWindow::createBandsButton() {
  ...
  connect(m_newReportHeader, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newReportFooter, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newPageHeader, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newPageFooter, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newData, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newDataHeader, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newDataFooter, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newSubDetail, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newSubDetailHeader, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newSubDetailFooter, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newGroupHeader, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  ...
  connect(m_newGroupFooter, &QAction::triggered, m_bandsAddSignalsMap, qOverload<>(&QSignalMapper::map));
  m_bandsAddSignalsMap->setMapping(m_newGroupFooter, BandDesignIntf::GroupFooter);
  m_newBandButton->addAction(m_newGroupFooter);

  connect(m_bandsAddSignalsMap, qOverload<int>(&QSignalMapper::mapped), this,
          qOverload<int>(&ReportDesignWindow::slotNewBand));
}
...
void ReportDesignWindow::createMainMenu() {
  ...
  connect(m_recentFilesSignalMap, qOverload<const QString &>(&QSignalMapper::mapped), this,
          &ReportDesignWindow::slotLoadRecentFile);
...
void ReportDesignWindow::initReportEditor(ReportEnginePrivate *report) {
  ...
  connect(m_reportDesignWidget, &ReportDesignWidget::itemPropertyChanged, this,
          &ReportDesignWindow::slotItemPropertyChanged);
  connect(m_reportDesignWidget, &ReportDesignWidget::insertModeStarted, this,
          &ReportDesignWindow::slotInsertModeStarted);

  connect(m_reportDesignWidget, &ReportDesignWidget::multiItemSelected, this,
          &ReportDesignWindow::slotMultiItemSelected);
  connect(m_reportDesignWidget, &ReportDesignWidget::itemInserted, this, &ReportDesignWindow::slotItemInserted);
  connect(m_reportDesignWidget, &ReportDesignWidget::itemInsertCanceled, this,
          &ReportDesignWindow::slotItemInsertCanceled);
  connect(m_reportDesignWidget->report(), &ReportEnginePrivate::datasourceCollectionLoadFinished, this,
          &ReportDesignWindow::slotUpdateDataBrowser);
  connect(m_reportDesignWidget, &ReportDesignWidget::commandHistoryChanged, this,
          &ReportDesignWindow::slotCommandHistoryChanged);
  ...
  connect(m_reportDesignWidget->report(), &ReportEnginePrivate::renderStarted, this,
          &ReportDesignWindow::renderStarted);
  connect(m_reportDesignWidget->report(), &ReportEnginePrivate::renderPageFinished, this,
          &ReportDesignWindow::renderPageFinished);
  connect(m_reportDesignWidget->report(), &ReportEnginePrivate::renderFinished, this,
          &ReportDesignWindow::renderFinished);
}
...
void ReportDesignWindow::createRecentFilesMenu() {
  if (m_recentFilesMenu) {
    ...
    foreach (QString fileName, m_recentFiles.keys()) {
      ...
      connect(tmpAction, &QAction::triggered, m_recentFilesSignalMap, qOverload<>(&QSignalMapper::map));
...
bool ReportDesignWindow::checkNeedToSave() {
  if (m_reportDesignWidget->isNeedToSave()) {
    QMessageBox::StandardButton button =
        QMessageBox::question(this, "", tr("Report has been modified ! Do you want save the report ?"),
...
void ReportDesignWindow::slotItemSelected(LimeReport::BaseDesignIntf *item) {
  if (m_propertyModel->currentObject() != item) {
    ...
    if (band) {
      ...
      if ((band->bandType() == BandDesignIntf::Data) or (band->bandType() == BandDesignIntf::SubDetailBand)) {
...
void ReportDesignWindow::slotItemPropertyChanged(const QString &objectName, const QString &propertyName,
                                                 const QVariant &oldValue, const QVariant &newValue) {
  Q_UNUSED(oldValue)
  Q_UNUSED(newValue)

  if (m_propertyModel->currentObject() and (m_propertyModel->currentObject()->objectName() == objectName)) {
...
void ReportDesignWindow::slotLoadReport() {
  if (checkNeedToSave()) {
    if (!m_reportDesignWidget->emitLoadReport()) {
      QString fileName = QFileDialog::getOpenFileName(this, tr("Report file name"),
...
void ReportDesignWindow::slotPreviewReport() { m_reportDesignWidget->report()->previewReport(); }
...
bool ReportDesignWindow::isDockAreaVisible(Qt::DockWidgetArea area) {
  ...
  foreach (QDockWidget *dw, dockWidgets) {
    if ((dockWidgetArea(dw) == area) and !dw->isHidden()) return true;
  }
  return false;
}
