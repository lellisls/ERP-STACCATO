...
ReportDesignWidget::ReportDesignWidget(ReportEnginePrivate *report, QMainWindow *mainWindow, QWidget *parent)
  ...
  for (QGraphicsItem *item : activePage()->selectedItems()) item->setSelected(false);
  connect(m_report, &ReportEnginePrivate::pagesLoadFinished, this, &ReportDesignWidget::slotPagesLoadFinished);
  connect(m_report, &ReportEnginePrivate::cleared, this, &ReportDesignWidget::cleared);
...
void ReportDesignWidget::setActivePage(PageDesignIntf *page) {
  ...
  connect(page, &PageDesignIntf::itemInserted, this, &ReportDesignWidget::itemInserted);
  connect(page, &PageDesignIntf::itemInsertCanceled, this, &ReportDesignWidget::itemInsertCanceled);
  connect(page, &PageDesignIntf::itemPropertyChanged, this, &ReportDesignWidget::itemPropertyChanged);
  connect(page, &QGraphicsScene::selectionChanged, this, &ReportDesignWidget::slotSelectionChanged);
  connect(page, &PageDesignIntf::insertModeStarted, this, &ReportDesignWidget::insertModeStarted);
  connect(page, &PageDesignIntf::commandHistoryChanged, this, &ReportDesignWidget::commandHistoryChanged);
  connect(page, &QGraphicsScene::sceneRectChanged, this, &ReportDesignWidget::slotSceneRectChanged);

  connect(page, &PageDesignIntf::itemAdded, this, &ReportDesignWidget::itemAdded);
  connect(page, &PageDesignIntf::itemRemoved, this, &ReportDesignWidget::itemDeleted);
  connect(page, &PageDesignIntf::bandAdded, this, &ReportDesignWidget::bandAdded);
  connect(page, &PageDesignIntf::bandRemoved, this, &ReportDesignWidget::bandDeleted);
  connect(page, &PageDesignIntf::pageUpdateFinished, this, &ReportDesignWidget::activePageUpdated);
...
void ReportDesignWidget::addBand(const QString &bandType) { activePage()->addBand(bandType); }

void ReportDesignWidget::addBand(BandDesignIntf::BandsType bandType) { activePage()->addBand(bandType); }

void ReportDesignWidget::startInsertMode(const QString &itemType) { activePage()->startInsertMode(itemType); }

void ReportDesignWidget::startEditMode() { activePage()->startEditMode(); }

PageDesignIntf *ReportDesignWidget::activePage() { return qobject_cast<PageDesignIntf *>(m_view->scene()); }

QList<QGraphicsItem *> ReportDesignWidget::selectedItems() { return m_view->scene()->selectedItems(); }
...
void ReportDesignWidget::saveToFile(const QString &fileName) { m_report->saveToFile(fileName); }
...
bool ReportDesignWidget::save() {
  if (!m_report->reportFileName().isEmpty()) {
    ...
    if (m_report->saveToFile(
            QFileDialog::getSaveFileName(this, tr("Report file name"), "", "Report files (*.lrxml);; All files (*)"))) {
      m_report->emitSaveFinished();
      return true;
    };
  }
...
void ReportDesignWidget::scale(qreal sx, qreal sy) { m_view->scale(sx, sy); }

QString ReportDesignWidget::reportFileName() { return m_report->reportFileName(); }

bool ReportDesignWidget::isNeedToSave() { return m_report->isNeedToSave(); }

bool ReportDesignWidget::emitLoadReport() { return m_report->emitLoadReport(); }

void ReportDesignWidget::updateSize() { activePage()->slotUpdateItemSize(); }

void ReportDesignWidget::undo() { activePage()->undo(); }

void ReportDesignWidget::redo() { activePage()->redo(); }

void ReportDesignWidget::copy() { activePage()->copy(); }

void ReportDesignWidget::paste() { activePage()->paste(); }

void ReportDesignWidget::cut() { activePage()->cut(); }

void ReportDesignWidget::brinToFront() { activePage()->bringToFront(); }

void ReportDesignWidget::sendToBack() { activePage()->sendToBack(); }

void ReportDesignWidget::alignToLeft() { activePage()->alignToLeft(); }

void ReportDesignWidget::alignToRight() { activePage()->alignToRigth(); }

void ReportDesignWidget::alignToVCenter() { activePage()->alignToVCenter(); }

void ReportDesignWidget::alignToTop() { activePage()->alignToTop(); }

void ReportDesignWidget::alignToBottom() { activePage()->alignToBottom(); }

void ReportDesignWidget::alignToHCenter() { activePage()->alignToHCenter(); }

void ReportDesignWidget::sameHeight() { activePage()->sameHeight(); }

void ReportDesignWidget::sameWidth() { activePage()->sameWidth(); }
...
void ReportDesignWidget::addHLayout() { activePage()->addHLayout(); }

void ReportDesignWidget::setFont(const QFont &font) { activePage()->setFont(font); }
...
void ReportDesignWidget::setBorders(const BaseDesignIntf::BorderLines &borders) { activePage()->setBorders(borders); }
...
bool ReportDesignWidget::isCanUndo() { return activePage()->isCanUndo(); }

bool ReportDesignWidget::isCanRedo() { return activePage()->isCanRedo(); }
...
void ReportDesignWidget::slotSceneRectChanged(QRectF) { m_view->centerOn(0, 0); }
...
void ReportDesignWidget::clear() { m_report->clearReport(); }
}
