...
BaseDesignIntf::BaseDesignIntf(const QString &storageTypeName, QObject *owner, QGraphicsItem *parent)
    : QObject(owner), QGraphicsItem(parent), m_resizeHandleSize(Const::RESIZE_HANDLE_SIZE),
      m_selectionPenSize(Const::SELECTION_PEN_SIZE),
      m_posibleResizeDirectionFlags(ResizeTop | ResizeBottom | ResizeLeft | ResizeRight),
      m_posibleMoveDirectionFlags(All), m_resizeDirectionFlags(0), m_width(200), m_height(50), m_fontColor(Qt::black),
      m_mmFactor(Const::mmFACTOR), m_fixedPos(false), m_borderLineSize(1), m_BGMode(OpaqueMode), m_opacity(100),
      m_borderLinesFlags(0), m_hintFrame(0), m_storageTypeName(storageTypeName), m_itemMode(DesignMode),
      m_objectState(ObjectCreated), m_selectionMarker(0), m_joinMarker(0), m_backgroundBrush(Solid),
      m_backgroundBrushcolor(Qt::white), m_margin(4), m_itemAlign(DesignedItemAlign), m_changingItemAlign(false),
...
void BaseDesignIntf::prepareRect(QPainter *ppainter, const QStyleOptionGraphicsItem * /*option*/,
                                 QWidget * /*widget*/) {
  ppainter->save();
  if (isSelected() and (opacity() == 100) and (m_BGMode != TransparentMode)) {
...
void BaseDesignIntf::hoverMoveEvent(QGraphicsSceneHoverEvent *event) {
  if (page()) {
    if (!page()->isItemInsertMode() and isSelected()) {
...
void BaseDesignIntf::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (m_resizeDirectionFlags & ResizeLeft) {
    if ((event->scenePos().x()) <= (mapToScene(0, 0).x() + (width() - Const::MINIMUM_ITEM_WIDTH)) and
  ...
  if (m_resizeDirectionFlags & ResizeRight) {
    if ((event->scenePos().x() >= (mapToScene(0, 0).x() + Const::MINIMUM_ITEM_WIDTH)) or
  ...
  if (m_resizeDirectionFlags & ResizeBottom) {
    if ((event->scenePos().y() > (mapToScene(0, 0).y() + height())) or
  ...
  if (m_resizeDirectionFlags & ResizeTop) {
    ...
    if ((event->scenePos().y()) <= (mapToScene(0, 0).y() + (height() - Const::MINIMUM_ITEM_HEIGHT)) and
  ...
  if ((m_resizeDirectionFlags == Fixed) and !m_fixedPos) {
    ...
    if (!isBand() and scene()->selectedItems().count() > 1) moveSelectedItems(tmpPos - pos());
    if (scene()->selectedItems().count() == 1 and (page()->magneticMovement())) page()->itemMoved(this);
  }
}

int BaseDesignIntf::posibleResizeDirectionFlags() const { return m_posibleResizeDirectionFlags; }
...
int BaseDesignIntf::resizeDirectionFlags(QPointF position) {
  ...
  if (posibleResizeDirectionFlags() & ResizeTop and m_topRect.contains(position)) {
    flags |= ResizeTop;
  }

  if (posibleResizeDirectionFlags() & ResizeLeft and m_leftRect.contains(position)) {
    flags |= ResizeLeft;
  }

  if (posibleResizeDirectionFlags() & ResizeBottom and m_bottomRect.contains(position)) {
    flags |= ResizeBottom;
  }

  if (posibleResizeDirectionFlags() & ResizeRight and m_rightRect.contains(position)) {
    flags |= ResizeRight;
  }
...
Qt::CursorShape BaseDesignIntf::getPosibleCursor(int cursorFlags) {
  ...
  if ((cursorFlags == Fixed) or (scene()->selectedItems().count() > 1)) return Qt::ArrowCursor;

  if (((cursorFlags & ResizeRight) and (cursorFlags & ResizeTop)) or
      ((cursorFlags & ResizeLeft) and (cursorFlags & ResizeBottom))) {
    return Qt::SizeBDiagCursor;
  }
  if (((cursorFlags & ResizeLeft) and (cursorFlags & ResizeTop)) or
      ((cursorFlags & ResizeRight) and (cursorFlags & ResizeBottom))) {
    return Qt::SizeFDiagCursor;
  }
  if ((cursorFlags & ResizeLeft) or (cursorFlags & ResizeRight)) {
    return Qt::SizeHorCursor;
  }
  if ((cursorFlags & ResizeBottom) or (cursorFlags & ResizeTop)) {
...
void BaseDesignIntf::turnOnSelectionMarker(bool value) {
  if (value and !m_selectionMarker) {
...
void BaseDesignIntf::sizeRight() {
  if ((m_posibleResizeDirectionFlags & ResizeLeft) or (m_posibleResizeDirectionFlags & ResizeRight)) {
...
void BaseDesignIntf::sizeUp() {
  if ((m_posibleResizeDirectionFlags & ResizeTop) or (m_posibleResizeDirectionFlags & ResizeBottom)) {
...
void BaseDesignIntf::drawBorder(QPainter *painter, QRectF rect) const {
  painter->save();
  if (itemMode() & DesignMode and drawDesignBorders()) {
...
void BaseDesignIntf::setGeometry(QRectF rect) {
  ...
  m_rect = rect;
  m_topRect =
      QRectF(0 - resizeHandleSize(), 0 - resizeHandleSize(), width() + resizeHandleSize() * 2, resizeHandleSize() * 2);
  m_bottomRect = QRectF(0 - resizeHandleSize(), height() - resizeHandleSize(), width() + resizeHandleSize() * 2,
                        resizeHandleSize() * 2);
  m_leftRect =
      QRectF(0 - resizeHandleSize(), 0 - resizeHandleSize(), resizeHandleSize() * 2, height() + resizeHandleSize() * 2);
  m_rightRect = QRectF(width() - resizeHandleSize(), 0 - resizeHandleSize(), resizeHandleSize() * 2,
...
void BaseDesignIntf::setGeometryProperty(QRectF rect) {
  if (rect != geometry()) {
    ...
    if ((rect.x() != geometry().x()) or (rect.y() != geometry().y())) setPos(rect.x(), rect.y());
...
void BaseDesignIntf::initFlags() {
  if ((itemMode() & DesignMode) or (itemMode() & EditMode)) {
...
void BaseDesignIntf::moveSelectedItems(QPointF delta) {
  ...
  foreach (QGraphicsItem *item, selectedItems) {
    if (item != this) {
      ...
      if (selectedItem and !selectedItem->isBand()) {
...
void BaseDesignIntf::showEditorDialog() {
  ...
  if (editor) {
    ...
    connect(editor, &QObject::destroyed, dialog, &QWidget::close);
    dialog->exec();
#endif
  }
}
...
void BaseDesignIntf::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event) {
  if (event->button() == Qt::LeftButton and ((itemMode() & EditMode) or (itemMode() & DesignMode))) {
    showEditorDialog();
  }
  QGraphicsItem::mouseDoubleClickEvent(event);
}

int BaseDesignIntf::posibleMoveDirectionFlags() const { return m_posibleMoveDirectionFlags; }

void BaseDesignIntf::setPosibleMoveFlags(int directionsFlags) { m_posibleMoveDirectionFlags = directionsFlags; }
...
void BaseDesignIntf::updateSelectionMarker() {
  if (m_selectionMarker and (itemMode() & DesignMode or itemMode() & EditMode)) {
    if ((!m_selectionMarker->scene()) and scene()) scene()->addItem(m_selectionMarker);
    if (parentItem()) {
...
QObject *BaseDesignIntf::createElement(const QString & /*collectionName*/, const QString &elementType) {
  ...
  try {
    if (LimeReport::DesignElementsFactory::instance().objectCreator(elementType)) {
      ...
      connect(obj, &BaseDesignIntf::propertyChanged, page(), &PageDesignIntf::slotItemPropertyChanged);
    }
...
void Marker::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
  ...
  painter->drawRect(QRectF(rect().left() - markerSize, rect().bottom() - rect().height() / 2 - markerSize,
                           markerSize * 2, markerSize * 2));
  painter->drawRect(QRectF(rect().right() - markerSize, rect().bottom() - rect().height() / 2 - markerSize,
                           markerSize * 2, markerSize * 2));
  painter->drawRect(QRectF(rect().left() + rect().width() / 2 - markerSize, rect().top() - markerSize, markerSize * 2,
                           markerSize * 2));
  painter->drawRect(QRectF(rect().left() + rect().width() / 2 - markerSize, rect().bottom() - markerSize,
                           markerSize * 2, markerSize * 2));
}
