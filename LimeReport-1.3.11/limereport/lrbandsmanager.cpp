...
QStringList BandsManager::bandNames() {
  ...
  bandsList << QObject::tr("Report Header") << QObject::tr("Page Header") << QObject::tr("Page Footer")
  ...
  foreach (ItemAttribs attr, LimeReport::DesignElementsFactory::instance().attribsMap().values()) {
    if ((attr.m_tag == LimeReport::Const::bandTAG) and (!bandsList.contains(attr.m_alias)))
