...
public:
  explicit VSegment(QRectF rect) : Segment(rect.top(), rect.bottom()) {}
};

struct HSegment : public Segment {
public:
  explicit HSegment(QRectF rect) : Segment(rect.left(), rect.right()) {}
};

struct ItemSortContainer {
  QRectF m_rect;
  BaseDesignIntf *m_item;
  explicit ItemSortContainer(BaseDesignIntf *item) {
    m_item = item;
