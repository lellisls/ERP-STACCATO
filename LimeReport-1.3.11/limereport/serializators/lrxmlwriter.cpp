...
bool XMLWriter::saveToFile(QString fileName) {
  if ((m_doc->childNodes().count() == 0) or fileName.isEmpty()) return false;
  QFile xmlFile(fileName);
...
void XMLWriter::saveProperty(QString name, QObject *item, QDomElement *node) {
  ...
  if (enumOrFlag(name, item))
    ...
    try {
      ...
      qDebug() << "class name =" << item->metaObject()->className() << "property name=" << name
               << " property type=" << item->property(name.toLatin1()).typeName() << exception.what();
    }
...
bool XMLWriter::enumOrFlag(QString name, QObject *item) {
  return item->metaObject()->property(item->metaObject()->indexOfProperty(name.toLatin1())).isFlagType() or
...
bool XMLWriter::replaceNode(QDomElement node, QObject *item) {
  ...
  while (!element.isNull()) {
    ...
    if (!objectName.isNull() and (objectName.text() == item->objectName())) {
