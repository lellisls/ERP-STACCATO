...
bool registred = LimeReport::DesignElementsFactory::instance().registerCreator(
    xmlTag, LimeReport::ItemAttribs(QObject::tr("HLayout"), LimeReport::Const::bandTAG), createHLayout);
}
...
void HorizontalLayout::hoverEnterEvent(QGraphicsSceneHoverEvent *event) {
  Q_UNUSED(event)
  //    if ((itemMode() & LayoutEditMode) or isSelected()){
...
void HorizontalLayout::geometryChangedEvent(QRectF newRect, QRectF) {
  ...
  relocateChildren();
  if (m_layoutType == Table and !m_isRelocating) {
    divideSpace();
  }
}
...
BaseDesignIntf *HorizontalLayout::cloneUpperPart(int height, QObject *owner, QGraphicsItem *parent) {
  ...
  foreach (BaseDesignIntf *item, childBaseItems()) {
    ...
    if ((item->geometry().top() < height) and (item->geometry().bottom() > height)) {
...
void HorizontalLayout::restoreChild(BaseDesignIntf *item) {
  ...
  connect(item, &QObject::destroyed, this, &HorizontalLayout::slotOnChildDestroy);
  connect(item, &BaseDesignIntf::geometryChanged, this, &HorizontalLayout::slotOnChildGeometryChanged);
  connect(item, &BaseDesignIntf::itemAlignChanged, this, &HorizontalLayout::slotOnChildItemAlignChanged);
...
bool HorizontalLayout::isEmpty() const {
  ...
  foreach (QGraphicsItem *qgItem, childItems()) {
    ...
    if (item and !item->content().isEmpty()) isEmpty = false;
    if (!item and dynamic_cast<BaseDesignIntf *>(qgItem)) allItemsIsText = false;
  }
  return (isEmpty and allItemsIsText);
}
...
void HorizontalLayout::addChild(BaseDesignIntf *item, bool updateSize) {
  ...
  connect(item, &QObject::destroyed, this, &HorizontalLayout::slotOnChildDestroy);
  connect(item, &BaseDesignIntf::geometryChanged, this, &HorizontalLayout::slotOnChildGeometryChanged);
  connect(item, &BaseDesignIntf::itemVisibleHasChanged, this, &HorizontalLayout::slotOnChildVisibleHasChanged);
...
bool HorizontalLayout::isNeedUpdateSize(RenderPass pass) const {
  foreach (QGraphicsItem *child, childItems()) {
    BaseDesignIntf *item = dynamic_cast<BaseDesignIntf *>(child);
    if (item and item->isNeedUpdateSize(pass)) return true;
  }
  return false;
}
...
BaseDesignIntf *HorizontalLayout::findNext(BaseDesignIntf *item) {
  ...
  for (int i = 0; i < m_children.count(); ++i) {
    if (m_children[i] == item and m_children.size() > i + 1) {
...
void HorizontalLayout::divideSpace() {
  ...
  foreach (BaseDesignIntf *item, m_children) { itemsSumSize += item->width(); }
...
void HorizontalLayout::slotOnChildItemAlignChanged(BaseDesignIntf *item, const BaseDesignIntf::ItemAlign &,
                                                   const BaseDesignIntf::ItemAlign &) {
