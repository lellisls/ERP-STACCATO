...
public:
  ...
  bool isValid() { return (!m_tagText.isEmpty()) and (m_beginPos > 0) and (m_endPos > 0); }
...
public:
  ...
  Symb() : m_text(""), m_pos(-1) {}
  bool isValid() { return (!m_text.isEmpty()) and (m_pos > 0); }
  bool isTag() { return isValid() and m_text.at(0) == '<'; }
...
class HtmlContext {
public:
  explicit HtmlContext(QString html);
  ~HtmlContext();
