...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_a_pagar_vencer_base` (`Data Pagamento` INT, `Cartao` INT, `Cheque` INT, `Boleto` INT, `Outros` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_a_pagar_vencidos_base` (`Data Pagamento` INT, `Cartao` INT, `Cheque` INT, `Boleto` INT, `Outros` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_a_receber_vencer_base` (`Data Pagamento` INT, `Cartao` INT, `Cheque` INT, `Boleto` INT, `Outros` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_a_receber_vencidos_base` (`Data Pagamento` INT, `Cartao` INT, `Cheque` INT, `Boleto` INT, `Outros` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_conta_pagar` (`idPagamento` INT, `idLoja` INT, `contraparte` INT, `dataPagamento` INT, `dataEmissao` INT, `ordemCompra` INT, `numeroNFe` INT, `valor` INT, `tipo` INT, `parcela` INT, `observacao` INT, `status` INT, `statusFinanceiro` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_estoque` (`status` INT, `idEstoque` INT, `fornecedor` INT, `descricao` INT, `restante` INT, `restante est` INT, `un` INT, `un2` INT, `Caixas` INT, `codComercial` INT, `idCompra` INT, `nfe` INT, `dataPrevColeta` INT, `dataRealColeta` INT, `dataPrevReceb` INT, `dataRealReceb` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_caixa` (`Código` INT, `Data Pag` INT, `R$` INT, `Tipo` INT, `contaDestino` INT, `idConta` INT, `Status` INT, `ENT/SAI` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fluxo_resumo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_resumo` (`Data Pag` INT, `Conta` INT, `idConta` INT, `SAIDA` INT, `ENTRADA` INT, `R$` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_nfe_entrada` (`Fornecedor` INT, `NFe` INT, `Recebido Por` INT, `Data Receb` INT, `OC` INT, `Venda` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_nfe_saida`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_nfe_saida` (`chaveAcesso` INT, `NFe` INT, `Status` INT, `Venda` INT, `CPF/CNPJ` INT, `Cliente` INT, `Criado em` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_relatorio_nfe` (`NFe` INT, `Serie` INT, `DataEmi` INT, `produtos` INT, `icmsst` INT, `frete` INT, `seguro` INT, `ipi` INT, `outrasdesp` INT, `desconto` INT, `totalnfe` INT, `bcicms` INT, `icms` INT, `tipo` INT, `e/s` INT, `nprotocolo` INT, `dataautor` INT);
...
VIEW `mydb`.`view_entrega_pendente` AS
    SELECT 
        ...
        `vp`.`idVenda` AS `idVenda`,
        SUM(((`vp`.`status` = 'ESTOQUE')
            OR (`vp`.`status` = 'DEVOLVIDO'))) AS `Estoque`,
        SUM((`vp`.`status` = 'ENTREGUE')) AS `Entregue`,
        SUM(((`vp`.`status` <> 'ESTOQUE')
        ...
        (((SELECT 
            `e`.`status` AS `status`,
                ...
                (`e`.`quant` + COALESCE(SUM(IF(((`vp`.`status` = 'ENTREGUE')
                    OR (`vp`.`status` = 'EM ENTREGA')), `ec`.`quant`, 0)))) AS `Quant Est`,
                `p`.`un` AS `un`,
                `p`.`un2` AS `un2`,
...
VIEW `mydb`.`view_fluxo_resumo` AS
    SELECT 
        ...
        `view_fluxo_caixa`.`contaDestino` AS `Conta`,
        `view_fluxo_caixa`.`idConta` AS `idConta`,
        SUM(IF((`view_fluxo_caixa`.`ENT/SAI` = 'SAIDA'),
            `view_fluxo_caixa`.`R$`,
            0)) AS `SAIDA`,
        SUM(IF((`view_fluxo_caixa`.`ENT/SAI` = 'ENTRADA'),
            `view_fluxo_caixa`.`R$`,
            0)) AS `ENTRADA`,
        SUM(`view_fluxo_caixa`.`R$`) AS `R$`
    FROM
        `mydb`.`view_fluxo_caixa`
    GROUP BY `view_fluxo_caixa`.`Data Pag` , `view_fluxo_caixa`.`contaDestino`
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_orcamento` AS select `u`.`idLoja` AS `idLoja`,`u`.`idUsuario` AS `idUsuario`,`o`.`status` AS `Status`,(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))) AS `Dias restantes`,`o`.`idOrcamento` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Indicou`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`c`.`tel` AS `Tel`,`c`.`telCel` AS `Cel`,`c`.`telCom` AS `Tel Com`,`o`.`data` AS `Data`,`o`.`total` AS `Total`,`fl1`.`dataFollowup` AS `Data Followup`,`fl1`.`dataProxFollowup` AS `Data Próx Followup`,`fl1`.`observacao` AS `Observação` from ((((((`mydb`.`orcamento` `o` left join `mydb`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `mydb`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `mydb`.`orcamento_has_followup` `fl1` on((`fl1`.`idOrcamento` like concat(left(`o`.`idOrcamento`,11),'%')))) left join `mydb`.`orcamento_has_followup` `fl2` on(((`fl1`.`idOrcamento` like concat(left(`fl2`.`idOrcamento`,11),'%')) and (`fl1`.`dataProxFollowup` < `fl2`.`dataProxFollowup`)))) left join `mydb`.`usuario` `u2` on((`o`.`idUsuarioIndicou` = `u2`.`idUsuario`))) where isnull(`fl2`.`idOrcamento`);
