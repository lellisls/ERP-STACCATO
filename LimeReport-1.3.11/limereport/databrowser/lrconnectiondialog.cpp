...
ConnectionDialog::ConnectionDialog(LimeReport::IConnectionController *conControl,
    ...
    : QDialog(parent), ui(new Ui::ConnectionDialog), m_connection(connectionDesc), m_controller(conControl) {
  ui->setupUi(this);
...
void ConnectionDialog::init() { ui->cbbDrivers->addItems(QSqlDatabase::drivers()); }
...
void ConnectionDialog::checkFieldsFill() {
  ...
  if (!m_changeMode and QSqlDatabase::connectionNames().contains(ui->leConnectionName->text())) {
