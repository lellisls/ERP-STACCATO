...
TextItemEditor::TextItemEditor(LimeReport::TextItem *item, LimeReport::PageDesignIntf *page, QSettings *settings,
                               QWidget *parent)
    : QWidget(parent), ui(new Ui::TextItemEditor), m_textItem(item), m_page(page), m_settings(settings),
      m_ownedSettings(false), m_isReadingSetting(false) {
  ui->setupUi(this);
  initUI();
...
void TextItemEditor::initUI() {
  ...
  if (dm) {
    ...
    foreach (QString dsName, dm->dataSourceNames()) {
      foreach (QString field, dm->fieldNames(dsName)) { dataWords << dsName + "." + field; }
    }
  } else {
  ...
  if (ui->twScriptEngine->selectionModel()) {
    connect(ui->twScriptEngine->selectionModel(), &QItemSelectionModel::currentChanged, this,
            &TextItemEditor::slotScriptItemsSelectionChanged);
  }

  BandDesignIntf *band = findParentBand();
  if (band and ui->twData->model() and !band->datasourceName().isEmpty()) {
...
void CompleaterTextEditor::setCompleter(QCompleter *value) {
  ...
  connect(m_compleater, qOverload<const QString &>(&QCompleter::activated), this,
          &CompleaterTextEditor::insertCompletion);
}

void CompleaterTextEditor::keyPressEvent(QKeyEvent *e) {
  if (m_compleater and m_compleater->popup()->isVisible()) {
    switch (e->key()) {
    case Qt::Key_Enter:
    case Qt::Key_Return:
    case Qt::Key_Escape:
  ...
  bool isShortcut = ((e->modifiers() & Qt::ControlModifier) and e->key() == Qt::Key_E);
  if (!m_compleater or !isShortcut) QTextEdit::keyPressEvent(e);

  const bool ctrlOrShift = e->modifiers() & (Qt::ControlModifier | Qt::ShiftModifier);
  if (!m_compleater or (ctrlOrShift and e->text().isEmpty())) return;

  static QString eow("~!@#$%^&*()_+{}|:\"<>?,./;'[]\\-="); // end of word
  bool hasModifier = (e->modifiers() != Qt::NoModifier) and !ctrlOrShift;

  QString completionPrefix = textUnderCursor();

  if (!isShortcut and
      (hasModifier or e->text().isEmpty() or completionPrefix.length() < 3 or eow.contains(e->text().right(1)))) {
