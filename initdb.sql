...
CREATE TABLE IF NOT EXISTS `mydb`.`conta_a_receber_has_pagamento` (
  `idPagamento` INT(11) NOT NULL AUTO_INCREMENT,
  `dataEmissao` DATE NULL DEFAULT NULL,
  `idVenda` VARCHAR(30) NOT NULL DEFAULT NULL,
  `idLoja` INT(10) UNSIGNED NOT NULL,
  `contraParte` VARCHAR(90) NULL DEFAULT NULL,
  `valor` DECIMAL(15,4) NOT NULL,
  `tipo` VARCHAR(45) NOT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_agendar_coleta` (`idEstoque` INT, `fornecedor` INT, `numeroNFe` INT, `Transportadora` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `kgcx` INT, `idVenda` INT, `ordemCompra` INT, `local` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_coleta`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_coleta` (`idEstoque` INT, `fornecedor` INT, `numeroNFe` INT, `Transportadora` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `kgcx` INT, `idVenda` INT, `ordemCompra` INT, `local` INT, `dataPrevColeta` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_compras`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_compras` (`OC` INT, `Compra` INT, `Venda` INT, `Fornecedor` INT, `Vendedor` INT, `Data` INT, `Produtos` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_entrega` (`idEstoque` INT, `fornecedor` INT, `numeroNFe` INT, `Transportadora` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `kgcx` INT, `idVenda` INT, `ordemCompra` INT, `local` INT, `dataPrevEnt` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_entrega_base`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_entrega_base` (`idVenda` INT, `Estoque` INT, `Entregue` INT, `Outros` INT, `Bairro` INT, `Logradouro` INT, `Número` INT, `Cidade` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_faturamento` (`OC` INT, `Código` INT, `Fornecedor` INT, `Vendedor` INT, `Data` INT, `Produtos` INT, `Total` INT, `idCompra` INT, `representacao` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_orcamento` (`idLoja` INT, `idUsuario` INT, `Código` INT, `Vendedor` INT, `Indicou` INT, `Cliente` INT, `Profissional` INT, `Tel` INT, `Cel` INT, `Tel Com` INT, `Data` INT, `Total` INT, `Status` INT, `Dias restantes` INT, `Data Próx Followup` INT, `Observação` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_recebimento` (`idEstoque` INT, `fornecedor` INT, `numeroNFe` INT, `produto` INT, `quant` INT, `un` INT, `caixas` INT, `idVenda` INT, `codComercial` INT, `ordemCompra` INT, `local` INT, `dataPrevReceb` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda` (`idLoja` INT, `idUsuario` INT, `Código` INT, `Vendedor` INT, `Indicou` INT, `Cliente` INT, `Data` INT, `Total R$` INT, `Status Financeiro` INT, `Status` INT, `statusEntrega` INT, `Dias restantes` INT, `Data Próx Followup` INT, `Observação` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_venda_produto`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda_produto` (`data` INT, `fornecedor` INT, `idVenda` INT, `Vendedor` INT, `produto` INT, `caixas` INT, `quant` INT, `un` INT, `codComercial` INT, `formComercial` INT, `status` INT, `Financeiro` INT);
...
BEGIN 
    ...
    UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'DEVOLVIDO' AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'ESTOQUE'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'EM RECEBIMENTO'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'EM COLETA'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'EM FATURAMENTO'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'EM COMPRA'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
	UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'INICIADO'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
    UPDATE venda v, venda_has_produto vp SET v.status = vp.status WHERE v.idVenda = vp.idVenda AND vp.status = 'PENDENTE'AND v.status != 'CANCELADO' AND v.idVenda NOT LIKE '%D';
...
VIEW `mydb`.`view_entrega` AS
    ...
    WHERE
        ((`e`.`status` = 'ESTOQUE')
            AND (`pf`.`dataPrevEnt` IS NOT NULL)
            AND (`pf`.`idPedido` IS NOT NULL))
    GROUP BY `e`.`idEstoque` DESC;

-- -----------------------------------------------------
-- View `mydb`.`view_entrega_base`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `mydb`.`view_entrega_base` ;
DROP TABLE IF EXISTS `mydb`.`view_entrega_base`;
USE `mydb`;
CREATE 
     OR REPLACE ALGORITHM = UNDEFINED 
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `mydb`.`view_entrega_base` AS
    SELECT 
...
VIEW `mydb`.`view_fornecedor_logistica` AS
    SELECT 
        ...
        SUM((`e`.`status` = 'EM COLETA')) AS `COLET`,
        SUM((`e`.`status` = 'EM RECEBIMENTO')) AS `RECEB`
    FROM
        ((`mydb`.`estoque` `e`
        LEFT JOIN `mydb`.`produto` `p` ON ((`e`.`idProduto` = `p`.`idProduto`)))
...
VIEW `mydb`.`view_venda` AS
    SELECT 
        ...
        `v`.`data` AS `Data`,
        `v`.`total` AS `Total R$`,
        `v`.`statusFinanceiro` AS `Status Financeiro`,
        `v`.`status` AS `Status`,
