...
void ObjectInspectorWidget::drawRow(QPainter *painter, const QStyleOptionViewItem &options,
                                    const QModelIndex &index) const {
  ObjectPropItem *node = nodeFromIndex(index);
  QStyleOptionViewItem so = options;
  ...
  if (node) {
    if ((!node->isHaveValue())) {
      ...
      if (index.isValid() and (nodeFromIndex(index)->colorIndex() != -1)) {
  ...
  if ((event->button() == Qt::LeftButton)) {
    ...
    if (index.isValid()) {
      if (event->pos().x() < indentation()) {
        ...
        if ((index.column() == 1) and (!nodeFromIndex(index)->isHaveChildren())) {
