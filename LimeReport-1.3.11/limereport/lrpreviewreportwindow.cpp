...
PreviewReportWindow::PreviewReportWindow(ReportEnginePrivate *report, QWidget *parent, QSettings *settings,
  ...
  connect(m_pagesNavigator, qOverload<int>(&QSpinBox::valueChanged), this,
          &PreviewReportWindow::slotPageNavigatorChanged);
  m_previewReportWidget = new PreviewReportWidget(report, this);
  setCentralWidget(m_previewReportWidget);
  layout()->setContentsMargins(1, 1, 1, 1);
  connect(m_previewReportWidget, &PreviewReportWidget::pageChanged, this, &PreviewReportWindow::slotPageChanged);

  m_scalePercent = new QComboBox(this);
  m_scalePercent->setEditable(true);
  ui->toolBar->insertWidget(ui->actionZoomOut, m_scalePercent);
  initPercentCombobox();
  connect(m_previewReportWidget, &PreviewReportWidget::scalePercentChanged, this,
          &PreviewReportWindow::slotScalePercentChanged);
  connect(m_scalePercent, qOverload<const QString &>(&QComboBox::currentIndexChanged), this,
          &PreviewReportWindow::scaleComboboxChanged);
  restoreSetting();
}
