...
VIEW `mydb`.`view_fornecedor_compra` AS
    SELECT 
        ...
        END)) AS `PEND`,
        SUM((CASE
            WHEN (`mydb`.`pedido_fornecedor_has_produto`.`status` = 'INICIADO') THEN 1
            ELSE 0
        END)) AS `INIC`,
        SUM((CASE
...
VIEW `mydb`.`view_fornecedor_logistica` AS
    SELECT 
        `mydb`.`pedido_fornecedor_has_produto`.`fornecedor` AS `fornecedor`,
        COUNT(`mydb`.`pedido_fornecedor_has_produto`.`fornecedor`) AS `COUNT(fornecedor)`
    FROM
        `mydb`.`pedido_fornecedor_has_produto`
    WHERE
    ...
    SELECT 
        ...
        (SELECT 
            ...
            FROM
                `mydb`.`estoque`
            WHERE
                (`mydb`.`estoque`.`codComercial` = `v`.`codComercial`)) AS `quant`
    FROM
