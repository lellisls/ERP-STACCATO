...
CREATE TABLE IF NOT EXISTS `mydb`.`estoque` (
  ...
  `ordemCompra_` INT(11) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `mydb`.`estoque_has_consumo` (
  ...
  `status` VARCHAR(45) NULL DEFAULT 'TEMP',
  `ordemCompra_` INT(11) NULL DEFAULT NULL,
  `idCompra_` INT(11) NOT NULL DEFAULT '0',
  `local` VARCHAR(100) NULL DEFAULT 'TEMP',
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_caminhao` (`idVeiculo` INT, `Transportadora` INT, `Veículo` INT, `Placa` INT, `Evento` INT, `Produtos` INT, `Estoque` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_estoque` (`status` INT, `idEstoque` INT, `fornecedor` INT, `descricao` INT, `restante` INT, `un` INT, `un2` INT, `Caixas M2` INT, `Caixas PÇ` INT, `codComercial` INT, `nfe` INT, `dataPrevColeta` INT, `dataRealColeta` INT, `dataPrevReceb` INT, `dataRealReceb` INT, `dataPrevEnt` INT, `dataRealEnt` INT);
...
VIEW `mydb`.`view_estoque` AS
    SELECT 
        ...
        `x`.`un` AS `un`,
        `x`.`un2` AS `un2`,
        `x`.`Caixas M2` AS `Caixas M2`,
        `x`.`Caixas PÇ` AS `Caixas PÇ`,
        ...
        `y`.`dataPrevEnt` AS `dataPrevEnt`,
        `y`.`dataRealEnt` AS `dataRealEnt`
    FROM
        (((SELECT 
            `e`.`status` AS `status`,
                `e`.`idEstoque` AS `idEstoque`,
...
VIEW `mydb`.`view_financeiro` AS
    SELECT 
        ...
        (SUM(IF(ISNULL(`estq`.`valorunid`),
            ...
            ((`mydb`.`venda_has_produto`.`quant` * (1 - (`estq`.`desconto` / `estq`.`valor`))) * `estq`.`valorunid`))) + 0E0) AS `custo`,
