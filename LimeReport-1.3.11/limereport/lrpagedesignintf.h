...
public:
  explicit ItemProjections(BaseDesignIntf *item)
      : m_xProjection(item->pos().x(), item->pos().x() + item->width()),
        m_yProjection(item->pos().y(), item->pos().y() + item->height()), m_item(item) {}
...
public:
  static CommandIf::Ptr create(PageDesignIntf *page, const QString &objectName, const QString &propertyName,
...
public:
  static CommandIf::Ptr create(PageDesignIntf *page, const QString &objectName, BaseDesignIntf::ItemAlign oldValue,
