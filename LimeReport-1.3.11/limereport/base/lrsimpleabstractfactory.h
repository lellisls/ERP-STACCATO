...
public:
  bool registerCreator(const IdentifierType &id, Attribs attribs, ProductCreator creator) {
    return SimpleFactory::registerCreator(id, creator) and (m_attribsMap.insert(id, attribs).value() == attribs);
  }
  bool unregisterCreator(const IdentifierType &id) {
    return SimpleFactory::unregisterCreator(id) and (m_attribsMap.remove(id) == 1);
  }
